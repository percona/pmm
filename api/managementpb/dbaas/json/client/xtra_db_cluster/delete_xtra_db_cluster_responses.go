// Code generated by go-swagger; DO NOT EDIT.

package xtra_db_cluster

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"
	"strconv"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// DeleteXtraDBClusterReader is a Reader for the DeleteXtraDBCluster structure.
type DeleteXtraDBClusterReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *DeleteXtraDBClusterReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewDeleteXtraDBClusterOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil
	default:
		result := NewDeleteXtraDBClusterDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewDeleteXtraDBClusterOK creates a DeleteXtraDBClusterOK with default headers values
func NewDeleteXtraDBClusterOK() *DeleteXtraDBClusterOK {
	return &DeleteXtraDBClusterOK{}
}

/*DeleteXtraDBClusterOK handles this case with default header values.

A successful response.
*/
type DeleteXtraDBClusterOK struct {
	Payload interface{}
}

func (o *DeleteXtraDBClusterOK) Error() string {
	return fmt.Sprintf("[POST /v1/management/DBaaS/XtraDBCluster/Delete][%d] deleteXtraDbClusterOk  %+v", 200, o.Payload)
}

func (o *DeleteXtraDBClusterOK) GetPayload() interface{} {
	return o.Payload
}

func (o *DeleteXtraDBClusterOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewDeleteXtraDBClusterDefault creates a DeleteXtraDBClusterDefault with default headers values
func NewDeleteXtraDBClusterDefault(code int) *DeleteXtraDBClusterDefault {
	return &DeleteXtraDBClusterDefault{
		_statusCode: code,
	}
}

/*DeleteXtraDBClusterDefault handles this case with default header values.

An unexpected error response.
*/
type DeleteXtraDBClusterDefault struct {
	_statusCode int

	Payload *DeleteXtraDBClusterDefaultBody
}

// Code gets the status code for the delete xtra DB cluster default response
func (o *DeleteXtraDBClusterDefault) Code() int {
	return o._statusCode
}

func (o *DeleteXtraDBClusterDefault) Error() string {
	return fmt.Sprintf("[POST /v1/management/DBaaS/XtraDBCluster/Delete][%d] DeleteXtraDBCluster default  %+v", o._statusCode, o.Payload)
}

func (o *DeleteXtraDBClusterDefault) GetPayload() *DeleteXtraDBClusterDefaultBody {
	return o.Payload
}

func (o *DeleteXtraDBClusterDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(DeleteXtraDBClusterDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

/*DeleteXtraDBClusterBody delete xtra DB cluster body
swagger:model DeleteXtraDBClusterBody
*/
type DeleteXtraDBClusterBody struct {

	// Kubernetes cluster name.
	KubernetesClusterName string `json:"kubernetes_cluster_name,omitempty"`

	// XtraDB cluster name.
	Name string `json:"name,omitempty"`
}

// Validate validates this delete xtra DB cluster body
func (o *DeleteXtraDBClusterBody) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (o *DeleteXtraDBClusterBody) MarshalBinary() ([]byte, error) {
	if o == nil {
		return nil, nil
	}
	return swag.WriteJSON(o)
}

// UnmarshalBinary interface implementation
func (o *DeleteXtraDBClusterBody) UnmarshalBinary(b []byte) error {
	var res DeleteXtraDBClusterBody
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*o = res
	return nil
}

/*DeleteXtraDBClusterDefaultBody delete xtra DB cluster default body
swagger:model DeleteXtraDBClusterDefaultBody
*/
type DeleteXtraDBClusterDefaultBody struct {

	// error
	Error string `json:"error,omitempty"`

	// code
	Code int32 `json:"code,omitempty"`

	// message
	Message string `json:"message,omitempty"`

	// details
	Details []*DetailsItems0 `json:"details"`
}

// Validate validates this delete xtra DB cluster default body
func (o *DeleteXtraDBClusterDefaultBody) Validate(formats strfmt.Registry) error {
	var res []error

	if err := o.validateDetails(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (o *DeleteXtraDBClusterDefaultBody) validateDetails(formats strfmt.Registry) error {

	if swag.IsZero(o.Details) { // not required
		return nil
	}

	for i := 0; i < len(o.Details); i++ {
		if swag.IsZero(o.Details[i]) { // not required
			continue
		}

		if o.Details[i] != nil {
			if err := o.Details[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("DeleteXtraDBCluster default" + "." + "details" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

// MarshalBinary interface implementation
func (o *DeleteXtraDBClusterDefaultBody) MarshalBinary() ([]byte, error) {
	if o == nil {
		return nil, nil
	}
	return swag.WriteJSON(o)
}

// UnmarshalBinary interface implementation
func (o *DeleteXtraDBClusterDefaultBody) UnmarshalBinary(b []byte) error {
	var res DeleteXtraDBClusterDefaultBody
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*o = res
	return nil
}
