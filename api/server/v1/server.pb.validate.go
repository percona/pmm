...
// Validate checks the field values on SettingLock with the rules defined in
// the proto definition for this message. If any rules are violated, the first
// error encountered is returned, or nil if there are no violations.
func (m *SettingLock) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on SettingLock with the rules defined in
// the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in SettingLockMultiError, or
// nil if none found.
func (m *SettingLock) ValidateAll() error {
	return m.validate(true)
}

func (m *SettingLock) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	// no validation rules for EnvironmentVariable

	if len(errors) > 0 {
		return SettingLockMultiError(errors)
	}

	return nil
}

// SettingLockMultiError is an error wrapping multiple validation errors
// returned by SettingLock.ValidateAll() if the designated constraints aren't met.
type SettingLockMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m SettingLockMultiError) Error() string {
	msgs := make([]string, 0, len(m))
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m SettingLockMultiError) AllErrors() []error { return m }

// SettingLockValidationError is the validation error returned by
// SettingLock.Validate if the designated constraints aren't met.
type SettingLockValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e SettingLockValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e SettingLockValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e SettingLockValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e SettingLockValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e SettingLockValidationError) ErrorName() string { return "SettingLockValidationError" }

// Error satisfies the builtin error interface
func (e SettingLockValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
...
var _ error = SettingLockValidationError{}
...
func (m *Settings) validate(all bool) error {
	...
	for idx, item := range m.GetLockedSettings() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, SettingsValidationError{
						field:  fmt.Sprintf("LockedSettings[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
