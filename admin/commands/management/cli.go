...
type RegisterCmd struct {
	...
	NodeType          string `name:"node-type" enum:"generic,container" default:"generic" help:"Node type, one of: generic, container (default: generic})"`
...
type addCommonFlags struct {
	...
	addHostFlag        string `name:"host" help:"Service hostname or IP address (overrides positional argument)"`
	addPortFlag        uint16 `name:"port" help:"Service port number (overrides positional argument)"`
...
type AddProxySqlCmd struct {
	// TODO "address" and "name" are switched in every command. Need to verify if it causes issues
	// It's due to Address being optional and thus cannot be preceeded by a required argument.
	Address             string `name:"address" arg:"" help:"ProxySQL address and port (default: 127.0.0.1:6032)"`
...
type AddPostgreSqlCmd struct {
	Address           string `name:"address" arg:"" help:"PostgreSQL address and port (default: 127.0.0.1:5432)"`
	...
	Username          string `name:"username" default:"admin" help:"PostgreSQL username"`
	Password          string `name:"password" default:"admin" help:"PostgreSQL password"`
	Database          string `name:"database" default:"admin" help:"PostgreSQL database"`
	AgentPassword     string `name:"agent-password" help:"Custom password for /metrics endpoint"`
	CredentialsSource string `name:"credentials-source" type:"existingfile" help:"Credentials provider"`
	NodeID            string `name:"node-id" help:"Node ID (default is autodetected)"`
	PMMAgentID        string `name:"pmm-agent-id" help:"The pmm-agent identifier which runs this instance (default is autodetected)"`
...
type AddMySqlCmd struct {
	Address           string `name:"address" arg:"" help:"MySQL address and port (default: 127.0.0.1:3306)"`
	...
	NodeID            string `name:"node-id" help:"Node ID (default is autodetected)"`
	PMMAgentID        string `name:"pmm-agent-id" help:"The pmm-agent identifier which runs this instance (default is autodetected)"`
	Username          string `name:"username" default:"admin" help:"MySQL username"`
	Password          string `name:"password" default:"admin" help:"MySQL password"`
	DefaultsFile      string `name:"defaults-file" default:"admin" help:"Path to defaults file"`
	AgentPassword     string `name:"agent-password" help:"Custom password for /metrics endpoint"`
	CredentialsSource string `name:"credentials-source" type:"existingfile" help:"Credentials provider"`
	...
	DisableQueryExamples bool   `name:"disable-queryexamples" help:"Disable collection of query examples"`
	// TODO check if works
	MaxSlowlogFileSize     units.Base2Bytes `name:"size-slow-logs" help:"Rotate slow log file at this size (default: server-defined; negative value disables rotation). Ex.: 1GiB"`
	...
	TLS                    bool             `name:"tls" help:"Use TLS to connect to the database"`
	TLSSkipVerify          bool             `name:"tls-skip-verify" help:"Skip TLS certificates validation"`
	TLSCaFile              string           `name:"tls-ca-file" help:"Path to certificate authority certificate file"`
	TLSCertFile            string           `name:"tls-cert-file" help:"Path to client certificate file"`
	TLSKeyFile             string           `name:"tls-key-file" help:"Path to client key file"`
...
type AddMongoDbCmd struct {
	Address           string `name:"address" arg:"" help:"MongoDB address and port (default: 127.0.0.1:27017)"`
	...
	CredentialsSource string `name:"credentials-source" type:"existingfile" help:"Credentials provider"`
	// TODO add "auto"
	QuerySource                   string `name:"query-source" default:"${mongoDbQuerySourceDefault}" enum:"${mongoDbQuerySourcesEnum}" help:"Source of SQL queries, one of: ${mongoDbQuerySourcesEnum} (default: ${mongoDbQuerySourceDefault})"`
	Environment                   string `name:"environment" help:"Environment name"`
	Cluster                       string `name:"cluster" help:"Cluster name"`
	ReplicationSet                string `name:"replication-set" help:"Replication set name"`
	CustomLabels                  string `name:"custom-labels" help:"Custom user-assigned labels"`
...
type AddHAProxyCmd struct {
	...
	ListenPort          uint16 `name:"listen-port" required:"" help:"Listen port of haproxy exposing the metrics for scraping metrics (Required)"`
	NodeID              string `name:"node-id" help:"Node ID (default is autodetected)"`
...
type AddExternalCmd struct {
	...
	Scheme              string `name:"scheme" placeholder:"http or https" help:"Scheme to generate URI to exporter metrics endpoints"`
	MetricsPath         string `name:"metrics-path" placeholder:"/metrics" help:"Path under which metrics are exposed, used to generate URI"`
	ListenPort          uint16 `name:"listen-port" required:"" help:"Listen port of external exporter for scraping metrics. (Required)"`
...
type AddExternalServerlessCmd struct {
	...
	Group               string `name:"group" default:"${externalDefaultGroupExporter}" help:"Group name of external service (default: ${externalDefaultGroupExporter})"`
	MachineID           string `name:"machine-id" default:"${defaultMachineID}" help:"Node machine-id"`
	Distro              string `name:"distro" default:"${distro}" help:"Node OS distribution"`
	ContainerID         string `name:"container-id" help:"Container ID"`
	ContainerName       string `name:"container-name" help:"Container name"`
	NodeModel           string `name:"node-model" help:"Node model"`
	Region              string `name:"region" help:"Node region"`
