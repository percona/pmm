...
func TestMySQLOptionsFromRequestExtraDSNParams(t *testing.T) {
	...
	t.Run("named time_zone is supported", func(t *testing.T) {
		opts, err := models.MySQLOptionsFromRequest(mysqlOptionsParamsStub{
			extraDSNParams: map[string]string{"time_zone": "'Europe/London'"},
		})
		require.NoError(t, err)
		assert.Equal(t, "'Europe/London'", opts.ExtraDSNParams["time_zone"])
	})
	...
	t.Run("malformed time_zone is rejected", func(t *testing.T) {
		for name, value := range map[string]string{
			"extra assignment": "'+00:00', sql_mode='STRICT_ALL_TABLES'",
			"semicolon":        "'+00:00'; SELECT 1",
			"embedded quote":   "'+00:00'' OR '1'='1'",
			"newline":          "'+00:00'\nSET time_zone='+01:00'",
			"carriage return":  "'+00:00'\rSET time_zone='+01:00'",
			"unquoted":         "+00:00",
			"empty":            "",
		} {
			t.Run(name, func(t *testing.T) {
				_, err := models.MySQLOptionsFromRequest(mysqlOptionsParamsStub{
					extraDSNParams: map[string]string{"time_zone": value},
				})
				require.Error(t, err)
				assert.Equal(t, codes.InvalidArgument, status.Code(err))
			})
