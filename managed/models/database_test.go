	...
	t.Run("Services", func(t *testing.T) {
		...
		_, err = db.Exec(
			"INSERT INTO services (service_id, service_type, service_name, node_id, environment, cluster, replication_set, address, port, socket, external_group, created_at, updated_at) "+
				"VALUES ('/service_id/1', 'mysql', 'name', '1', '', '', '', '10.10.10.10', 3306, '/var/run/mysqld/mysqld.sock', '', $1, $2)",
			now, now)
		...
		_, err = db.Exec(
			"INSERT INTO services (service_id, service_type, service_name, node_id, environment, cluster, replication_set, address, port, socket, external_group, created_at, updated_at) "+
				"VALUES ('/service_id/1', 'mysql', 'name', '1', '', '', '', NULL, NULL, NULL, '', $1, $2)",
			now, now)
		require.NoError(t, err)
		...
		_, err = db.Exec(
			"INSERT INTO services (service_id, service_type, service_name, node_id, environment, cluster, replication_set, address, port, socket, external_group, created_at, updated_at) "+
				"VALUES ('/service_id/1', 'mysql', 'name', '1', '', '', '', '10.10.10.10', 999999, NULL, '', $1, $2)",
			now, now)
		...
		_, err = db.Exec(
			"INSERT INTO services (service_id, service_type, service_name, node_id, environment, cluster, replication_set, address, port, socket, external_group, created_at, updated_at) "+
				"VALUES ('/service_id/1', 'external', 'name', '1', '', '', '', '10.10.10.10', 3333, NULL, '', $1, $2)",
			now, now)
		...
		_, err = db.Exec(
			"INSERT INTO services (service_id, service_type, service_name, node_id, environment, cluster, replication_set, address, port, socket, external_group, created_at, updated_at) "+
				"VALUES ('/service_id/1', 'mysql', 'name', '1', '', '', '', '10.10.10.10', 3306, NULL, 'non empty group', $1, $2)",
			now, now)
		require.Error(t, err, `pq: new row for relation "services" violates check constraint "services_external_group_check"`)
	})
	...
	t.Run("Agents", func(t *testing.T) {
		...
		_, err = db.Exec(
			"INSERT INTO services (service_id, service_type, service_name, node_id, environment, cluster, replication_set, socket, external_group, created_at, updated_at) "+
				"VALUES ('/service_id/1', 'mysql', 'name', '1', '', '', '', '/var/run/mysqld/mysqld.sock', '', $1, $2)",
			now, now)
		require.NoError(t, err)
		_, err = db.Exec(
		...
		t.Run("node_id_or_service_id_or_pmm_agent_id", func(t *testing.T) {
			...
			t.Run("service_id set", func(t *testing.T) {
				...
				_, err = tx.Exec(
					"INSERT INTO agents (agent_id, agent_type, runs_on_node_id, pmm_agent_id, node_id, service_id, disabled, status, created_at, updated_at, tls, tls_skip_verify, max_query_length, query_examples_disabled, comments_parsing_disabled, max_query_log_size, table_count_tablestats_group_limit, rds_basic_metrics_disabled, rds_enhanced_metrics_disabled, push_metrics, expose_exporter) "+
						"VALUES ('8', 'mysqld_exporter', NULL, '1', NULL, '/service_id/1', false, '', $1, $2, false, false, 0, false, true, 0, 0, false, false, false, false)",
					now, now)
			...
			t.Run("Both set", func(t *testing.T) {
				...
				_, err = tx.Exec(
					"INSERT INTO agents (agent_id, agent_type, runs_on_node_id, pmm_agent_id, node_id, service_id, disabled, status, created_at, updated_at, tls, tls_skip_verify, max_query_length, query_examples_disabled, comments_parsing_disabled, max_query_log_size, table_count_tablestats_group_limit, rds_basic_metrics_disabled, rds_enhanced_metrics_disabled, push_metrics, expose_exporter) "+
						"VALUES ('8', 'mysqld_exporter', NULL, '1', '1', '/service_id/1', false, '', $1, $2, false, false, 0, false, true, 0, 0, false, false, false, false)",
					now, now)
				assertCheckViolation(t, err, "agents", "node_id_or_service_id_for_non_pmm_agent")
			})
		})
