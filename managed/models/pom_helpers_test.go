...
func TestPomRuns(t *testing.T) {
	...
	insert := func(t *testing.T, q *reform.Querier, id string, offset time.Duration, document string) {
		...
		require.NoError(t, models.CreatePomRun(q, run, snapshot))
	}
	...
	t.Run("a run round-trips with its receipt and its document", func(t *testing.T) {
		...
		run, err := models.FindPomRunByID(q, "run-1")
		require.NoError(t, err)
		assert.Equal(t, models.PomRunSuccess, run.Status)
		...
		snapshot, err := models.FindLatestPomSnapshot(q)
		require.NoError(t, err)
	...
	t.Run("runs come back newest first", func(t *testing.T) {
		...
		runs, err := models.FindPomRuns(q, 2)
		require.NoError(t, err)
	...
	t.Run("pruning bounds the history and takes the documents with it", func(t *testing.T) {
		...
		require.NoError(t, models.PrunePomRuns(q, 3))

		runs, err := models.FindPomRuns(q, 100)
		require.NoError(t, err)
		...
		_, err = models.FindPomRunByID(q, "prune-0")
		require.ErrorIs(t, err, models.ErrNotFound)

		var count int
		require.NoError(t, db.QueryRow("SELECT count(*) FROM pom_snapshots").Scan(&count))
	...
	t.Run("absent things report absence, not an empty value", func(t *testing.T) {
		...
		_, err := models.FindPomRunByID(q, "nope")
		require.ErrorIs(t, err, models.ErrNotFound)

		_, err = models.FindPomRunByID(q, "")
		require.Error(t, err, "an empty id is a caller mistake, not a miss")

		runs, err := models.FindPomRuns(q, 0)
		require.NoError(t, err)
		assert.Empty(t, runs)

		require.Error(t, models.PrunePomRuns(q, 0), "keeping nothing is never what a caller meant")
	})
}

func TestPomSnapshotAbsentOnEmptyDatabase(t *testing.T) {
	sqlDB := testdb.Open(t, models.SkipFixtures, nil)
	t.Cleanup(func() { require.NoError(t, sqlDB.Close()) })
	...
	_, err := models.FindLatestPomSnapshot(db.Querier)
	require.ErrorIs(t, err, models.ErrNotFound)
}
