...
func TestNewInventoryMetricsCollector(t *testing.T) {
	...
	t.Run("Collector", func(t *testing.T) {
		...
		agentMetrics := []Metric{
			{
				labels: []string{"A1", string(models.PMMAgentType), "S1", "SN1", "N1", "NN1", "ENV1", "PA1", strconv.Itoa(1), "V1"},
				value:  float64(1),
			},
		}
		...
		const expectedAgentMetrics = `
			...
			pmm_managed_inventory_agents{agent_id="A1",agent_type="pmm-agent",disabled="1",environment="ENV1",node_id="N1",node_name="NN1",pmm_agent_id="PA1",service_id="S1",service_name="SN1",version="V1"} 1
		`
