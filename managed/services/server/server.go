...
func (s *Server) validateChangeSettingsRequest(ctx context.Context, req *serverpb.ChangeSettingsRequest) error {
	...
	if s.envSettings.MetricsResolutions.HR != 0 && metricsRes.GetHr().AsDuration() != s.envSettings.MetricsResolutions.HR {
	...
	if s.envSettings.MetricsResolutions.MR != 0 && metricsRes.GetMr().AsDuration() != s.envSettings.MetricsResolutions.MR {
	...
	if s.envSettings.MetricsResolutions.LR != 0 && metricsRes.GetLr().AsDuration() != s.envSettings.MetricsResolutions.LR {
	...
	if s.envSettings.DataRetention != 0 && req.DataRetention.AsDuration() != s.envSettings.DataRetention {
