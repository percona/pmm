...
func TestServiceService(t *testing.T) {
	...
	t.Run("RDS", func(t *testing.T) {
		...
		s.r.(*mockAgentsRegistry).On("IsConnected", mysqldExporter.AgentID).Return(false)
		s.r.(*mockAgentsRegistry).On("IsConnected", rdsExporter.AgentID).Return(false)

		response, err := s.ListServices(ctx, &managementpb.ListServiceRequest{})
		t.Logf("RDS Exporter ID: %s", rdsExporter.AgentID)
		t.Logf("Services: %+v", response.Services)
