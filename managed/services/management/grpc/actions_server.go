...
// StartPTMySQLChecksumAction starts pt-table-checksum action and returns the pointer to the response message
//nolint:lll
func (s *actionsServer) StartPTMySQLChecksumAction(ctx context.Context, req *managementpb.StartPTMySQLChecksumActionRequest) (*managementpb.StartPTMySQLChecksumActionResponse, error) {
	service, err := models.FindServiceByID(s.db.Querier, req.ServiceId)
	if err != nil {
		return nil, err
	}
	...
	case models.RemoteNodeType:
		...
		return nil, status.Errorf(codes.FailedPrecondition, "PTMySQL Checksum doesn't work with remote instances yet")
	...
	err = s.a.StartPTMySQLChecksumAction(ctx, res.ID, pmmAgentID, pointer.GetString(service.Address), pointer.GetUint16(service.Port),
	...
	return &managementpb.StartPTMySQLChecksumActionResponse{PmmAgentId: pmmAgentID, ActionId: res.ID}, nil
}
