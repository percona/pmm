...
func (s *ManagementService) addMongoDB(ctx context.Context, req *managementv1.AddMongoDBServiceParams) (*managementv1.AddServiceResponse, error) {
	...
	if e := s.db.InTransactionContext(ctx, nil, func(tx *reform.TX) error {
		...
		if req.MongodbRealtimeAnalytics {
			row, err = models.CreateAgent(tx.Querier, models.MongoDBRealtimeAnalyticsAgentType, &models.CreateAgentParams{
				PMMAgentID:    req.PmmAgentId,
				ServiceID:     service.ServiceID,
				Username:      req.Username,
				Password:      req.Password,
			...
			mongodb.MongodbRealtimeAnalytics = agent.(*inventoryv1.MongoDBRealtimeAnalyticsAgent) //nolint:forcetypeassert
		}
