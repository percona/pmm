...
// service_id matcher and %[2]s the freshness window; both aggregate by (service_id) so
// the result still joins back to a service.
//
...
//
// Sized against the scrape interval rather than picked: every metric in the catalog is
// collected by the high-resolution job, so the window is volatileMaxAgeFactor of PMM's
// configured high resolution, floored by minVolatileMaxAge. Four intervals tolerates
// several missed scrapes while still noticing a stopped database in well under a minute.
const (
...
// The document fields sources set. Names match SEP's pom_discovery app so a fact
...
// schemaVersion is the version of the emitted document. It began as the shape SEP's
// pom_api served, and is versioned so a stored snapshot stays readable across changes.
