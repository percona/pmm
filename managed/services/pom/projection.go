...
type document struct {
	summary      *pomv1.Summary
	environments []*pomv1.Environment
...
func buildDocument(services []*models.Service, merged map[string]map[string]MergedField, now time.Time, maxAge time.Duration) document {
	out := document{}
	grouped := make(map[string]map[string][]*pomv1.Service)
	...
	for _, service := range services {
		...
		if _, ok := grouped[env]; !ok {
			grouped[env] = make(map[string][]*pomv1.Service)
		}
		grouped[env][cluster] = append(grouped[env][cluster], serviceDocument(service, fields))
	}

	out.environments = make([]*pomv1.Environment, 0, len(grouped))
	for _, env := range sortedKeys(grouped) {
		clusters := make([]*pomv1.Cluster, 0, len(grouped[env]))
		for _, cluster := range sortedKeys(grouped[env]) {
			svcs := grouped[env][cluster]
			slices.SortFunc(svcs, func(a, b *pomv1.Service) int {
				return cmp.Compare(a.ServiceName, b.ServiceName)
			})
			clusters = append(clusters, &pomv1.Cluster{Name: named(cluster), Services: svcs})
		}
		out.environments = append(out.environments, &pomv1.Environment{EnvName: named(env), Clusters: clusters})
	}
...
func serviceDocument(service *models.Service, fields fieldSet) *pomv1.Service {
...
func buildSummary(environments []*pomv1.Environment) *pomv1.Summary {
	summary := &pomv1.Summary{
