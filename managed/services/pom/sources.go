...
// factSource is one place discovery reads the estate from.
//
// The interface is the extension point: the SEP discovery app's on-host facts arrive as
...
func (s metricsSource) collect(ctx context.Context, services []*models.Service) SourceResult {
	...
	for batch := range slicesChunk(ids, queryBatch) {
		matcher := fmt.Sprintf(`service_id=~%q`, strings.Join(batch, "|"))
