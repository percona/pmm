	...
	t.Run("RepoInfo", func(t *testing.T) {
		...
		stdout := strings.Split(`
			...
			* updates: mirror.netcologne.de
			...
			Repo-filename: /etc/yum.repos.d/pmm3-server.repo
...
func TestParseInfoEL9(t *testing.T) {
	...
	t.Run("Installed EL9", func(t *testing.T) {
		stdout := strings.Split(`
			...
			Unknown configuration option: async = 1 in /etc/yum.repos.d/pmm3-server.repo
	...
	t.Run("Updates EL9", func(t *testing.T) {
		...
		stdout := strings.Split(`
			...
			repo: using cache for: pmm3-server
			pmm3-server: using metadata from Wed 28 Jun 2023 02:46:09 PM UTC.
