...
export const detectQueryAnomalies = (
  ...
  if (avgTime > 1.0) {
    ...
    anomalies.push({
      ...
      recommendation:
        'Consider query optimization, adding indexes, or reviewing query logic.',
  ...
  if (rowsExamined > 0 && rowsSent > 0) {
    ...
    if (efficiency < 0.01 && rowsExamined > 1000) {
      // Less than 1% efficiency with significant rows
      anomalies.push({
        type: AnomalyType.EXCESSIVE_ROWS_EXAMINED,
        severity:
          efficiency < 0.001 ? AnomalySeverity.HIGH : AnomalySeverity.MEDIUM,
        description: `Poor query efficiency: examining ${rowsExamined.toLocaleString()} rows but returning only ${rowsSent.toLocaleString()}`,
        recommendation:
          'Add appropriate indexes or refine WHERE clauses to reduce rows examined.',
  ...
  if (lockTime > 0.1) {
    ...
    anomalies.push({
      ...
      recommendation:
        'Review transaction isolation levels, query timing, or consider query optimization.',
  ...
  if (queryRate > 10 && avgTime > 0.5) {
    // More than 10 QPS and > 500ms avg time
    anomalies.push({
      ...
      recommendation:
        'This query has high impact due to frequency. Prioritize optimization.',
  ...
  if (context.rank <= 5 && context.totalQueries.length > 10) {
    ...
    if (isOutlier) {
      anomalies.push({
        ...
        recommendation:
          'Investigate why this query behaves differently from others.',
        confidence: 0.7,
        metrics: {
  ...
  if (load > 10.0) {
    ...
    anomalies.push({
      ...
      recommendation:
        'High priority for optimization due to resource consumption.',
  ...
  const aiAnalysisPrompt =
    anomalies.length > 0
      ? generateAIAnalysisPrompt(query, anomalies)
      : undefined;
...
const detectPatternAnomalies = (fingerprint: string): QueryAnomaly[] => {
  ...
  if (fingerprint.includes('select')) {
    ...
    if (
      (fingerprint.match(/join/g) || []).length > 1 &&
      !fingerprint.includes('on')
    ) {
      anomalies.push({
        type: AnomalyType.CARTESIAN_JOIN,
        severity: AnomalySeverity.CRITICAL,
        description:
          'Potential cartesian join detected - multiple JOINs without proper ON conditions',
        recommendation:
          'Review JOIN conditions to ensure proper relationships.',
        confidence: 0.9,
      });
    }
    ...
    if (fingerprint.includes('order by') && !fingerprint.includes('limit')) {
      anomalies.push({
        ...
        recommendation:
          'Consider adding index on ORDER BY columns or add LIMIT clause.',
        confidence: 0.6,
      });
    }
  }
  ...
  if (
    ...
    fingerprint.includes('find(') ||
    fingerprint.includes('aggregate(')
  ) {
    ...
    if (fingerprint.includes('find({})') || fingerprint.includes('find()')) {
      anomalies.push({
        ...
        recommendation:
          'Add appropriate query filters and ensure indexes exist.',
        confidence: 0.9,
      });
    }
    ...
    if (fingerprint.includes('$lookup') || fingerprint.includes('$group')) {
      anomalies.push({
        ...
        severity: AnomalySeverity.MEDIUM,
        description:
          'Complex aggregation pipeline may benefit from optimization',
        recommendation:
          'Review aggregation stages and ensure appropriate indexes exist.',
        confidence: 0.7,
      });
    }
  }
...
const determineOverallSeverity = (
  ...
  const maxScore = Math.max(
    ...anomalies.map((a) => severityScores[a.severity])
  );
 ...
 */
const generateAIAnalysisPrompt = (
  query: QANRow,
  anomalies: QueryAnomaly[]
): string => {
...
export const analyzeQANReport = (
  ...
  const queryRows = qanData.rows.filter(
    (row) =>
  ...
  const criticalResults = anomalousResults.filter(
    (r) => r.result.overallSeverity === AnomalySeverity.CRITICAL
  );
  ...
  const topAnomalies = anomalousResults
    .sort((a, b) => {
      ...
      const aMaxConfidence = Math.max(
        ...a.result.anomalies.map((an) => an.confidence)
      );
      const bMaxConfidence = Math.max(
        ...b.result.anomalies.map((an) => an.confidence)
      );
...
export const detectQueryAnomaliesWithAI = async (
  ...
  try {
    ...
    console.warn(
      'AI anomaly detection failed, falling back to rule-based detection:',
      error
    );
 ...
 */
export const analyzeQANReportWithAI = async (
  qanData: QANReportResponse
): Promise<{
  totalQueries: number;
  anomalousQueries: number;
  ...
  try {
    ...
    const topAnomalies: Array<{
      query: QANRow;
      result: AnomalyDetectionResult;
    }> = [];

    for (const query of queryRows.slice(0, 20)) {
    ...
    topAnomalies.sort((a, b) => {
      ...
      const confidenceA =
        a.result.anomalies.reduce((sum, a) => sum + a.confidence, 0) /
        a.result.anomalies.length;
      const confidenceB =
        b.result.anomalies.reduce((sum, a) => sum + a.confidence, 0) /
        b.result.anomalies.length;
    ...
    console.warn(
      'AI batch analysis failed, falling back to rule-based analysis:',
      error
    );
