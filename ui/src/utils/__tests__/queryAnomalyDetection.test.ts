...
describe('Query Anomaly Detection', () => {
  describe('detectQueryAnomalies', () => {
    ...
    it('should detect excessive rows examined anomalies', () => {
      ...
      expect(
        result.anomalies.some(
          (a) => a.type === AnomalyType.EXCESSIVE_ROWS_EXAMINED
        )
      ).toBe(true);
    });
    ...
    it('should detect high lock time anomalies', () => {
      ...
      expect(result.hasAnomalies).toBe(true);
      expect(
        result.anomalies.some((a) => a.type === AnomalyType.HIGH_LOCK_TIME)
      ).toBe(true);
    });
    ...
    it('should detect full table scan patterns', () => {
      ...
      expect(result.hasAnomalies).toBe(true);
      expect(
        result.anomalies.some((a) => a.type === AnomalyType.FULL_TABLE_SCAN)
      ).toBe(true);
    });
    ...
    it('should detect high frequency slow queries', () => {
      ...
      expect(result.hasAnomalies).toBe(true);
      expect(
        result.anomalies.some((a) => a.type === AnomalyType.HIGH_FREQUENCY_SLOW)
      ).toBe(true);
      expect(result.overallSeverity).toBe(AnomalySeverity.HIGH);
    });
    ...
    it('should detect resource intensive queries', () => {
      ...
      expect(result.hasAnomalies).toBe(true);
      expect(
        result.anomalies.some((a) => a.type === AnomalyType.RESOURCE_INTENSIVE)
      ).toBe(true);
    ...
    it('should generate AI analysis prompts for anomalous queries', () => {
      ...
      expect(result.aiAnalysisPrompt).toContain(
        'Query Anomaly Analysis Request'
      );
