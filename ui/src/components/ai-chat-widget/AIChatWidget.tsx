...
type ChatAction =
  ...
  | {
      ...
      payload: (messages: ChatMessage[]) => ChatMessage[];
    }
...
export const AIChatWidget = forwardRef<AIChatWidgetRef, AIChatWidgetProps>(
  (
    ...
    const [state, dispatch] = useReducer(
      chatReducer,
      createInitialState(defaultOpen)
    );
    ...
    const loadChatHistory = async () => {
      try {
        ...
        console.log(
          '🔍 LoadChatHistory: Messages with attachments:',
    ...
    const loadAvailableTools = async (forceRefresh: boolean = false) => {
      try {
        ...
        console.log(
          `🔧 Loading MCP tools${forceRefresh ? ' (force refresh)' : ''}...`
        );

        // Log the exact URL being called
        const url = forceRefresh
          ? '/v1/chat/mcp/tools?force_refresh=true'
          : '/v1/chat/mcp/tools';
        ...
        console.log(
          `✅ ${toolsResponse.tools.length} MCP tools available:`,
    ...
    const createSessionOnFirstMessage =
      useCallback(async (): Promise<string> => {
        ...
        console.log(
          '✅ Session will be created automatically by backend on first message'
        );
        return newSessionId;
      }, []);
    ...
    const handleSessionSelect = async (newSessionId: string) => {
      console.log('🔄 Session select starting:', {
        newSessionId,
      ...
      try {
        ...
        dispatch({
          type: 'RESET_SESSION_STATE',
          payload: { sessionId: newSessionId, messages: processedMessages },
        });
      } catch (error) {
    ...
    const ensureSession = useCallback(async (): Promise<string> => {
      ...
      if (!state.sessionInitialized || !currentSessionId) {
        console.log(
          '🆕 No session exists, creating session on first message...'
        );
    ...
    const handleStreamMessage = useCallback(
      (streamMessage: StreamMessage, currentSessionId: string): string => {
        let updatedSessionId = currentSessionId;

        // Update session ID if backend provides a new one in stream messages
        if (
          streamMessage.session_id &&
          streamMessage.session_id !== currentSessionId
        ) {
          console.log(
            '🔄 Backend created new session via stream:',
            streamMessage.session_id
          );
          dispatch({
            type: 'SET_SESSION_ID',
            payload: streamMessage.session_id,
          });
        ...
        if (streamMessage.type === 'message') {
          ...
          dispatch({
            ...
            payload: streamingContentRef.current,
          });
          ...
          console.log(
            ...
            'Request ID:',
            streamMessage.request_id
          );
          if (
            streamMessage.tool_calls &&
            streamMessage.tool_calls.length > 0 &&
            streamMessage.request_id
          ) {
          ...
          if (
            streamMessage.tool_executions &&
            streamMessage.tool_executions.length > 0
          ) {
    ...
    const handleSendError = useCallback(
      (error: unknown, onComplete?: () => void): void => {
        ...
        const cleanup = await aiChatAPI.streamChatWithSeparateEndpoints(
          ...
          (streamMessage: StreamMessage) => {
            if (streamMessage.type === 'message') {
              ...
              streamMessage.type === 'tool_execution' &&
              decision === 'approve'
            ) {
              console.log(
                'Tool executions during approval:',
                streamMessage.tool_executions
              );
              if (
                streamMessage.tool_executions &&
    ...
    return (
      ...
      >
          ...
          <Toolbar variant="dense" sx={{ minHeight: 56 }}>
            ...
            <Box sx={{ display: 'flex', gap: 0.5 }}>
              ...
              >
                <IconButton
                  onClick={() =>
                    dispatch({ type: 'SET_SHOW_TOOLS', payload: true })
                  }
                  size="medium"
                  disabled={
              ...
              <Tooltip title="Refresh MCP Tools" arrow>
                ...
                >
                  <RefreshIcon
                    sx={{ fontSize: 20, color: '#ffffff', fontWeight: 'bold' }}
                  />
                </IconButton>
              </Tooltip>
              ...
              <Tooltip title="Chat Sessions" arrow>
                <IconButton
                  onClick={() =>
                    dispatch({
                      type: 'SET_SHOW_SESSION_SELECTOR',
                      payload: true,
                    })
                  }
                  size="medium"
        ...
        {(state.toolsLoading || state.availableTools.length > 0) && (
          ...
          >
            {state.toolsLoading ? (
              ...
              <>
                <ListIcon
                  sx={{ fontSize: 18, color: '#1976d2', fontWeight: 'bold' }}
                />
                <Typography
                  variant="body2"
                  sx={{ fontWeight: 600, color: '#1565c0' }}
                >
                  {state.availableTools.length} MCP tool
                  {state.availableTools.length !== 1 ? 's' : ''} available
                </Typography>
              </>
            )}
          </Box>
          ...
          {state.messages.length === 0 && !state.streamingMessage && (
            ...
            >
              ...
              <Typography variant="body2" color="textSecondary">
                Ask me anything! I can help with database queries, analysis, and
                more.
              </Typography>
              {!state.sessionId && (
                <Typography
        ...
        <SessionSelector
          ...
          onClose={() =>
            dispatch({ type: 'SET_SHOW_SESSION_SELECTOR', payload: false })
          }
