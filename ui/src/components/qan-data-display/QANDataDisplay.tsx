...
const QANDataDisplay: React.FC<QANDataDisplayProps> = ({
  ...
  const [queryAnomalies, setQueryAnomalies] = useState(
    new Map<string, AnomalyDetectionResult>()
  );
  ...
  useEffect(() => {
    if (!useAIDetection) {
      ...
      if (data.rows && data.rows.length > 0) {
        const queryRows = data.rows.filter(
          (row) =>
        ...
        const avgMetrics = queryRows.reduce(
          (acc, query) => {
            const avgTime =
      ...
      'batchAnalysis' in anomalyAnalysis &&
      anomalyAnalysis.batchAnalysis
    ) {
      ...
      if (batchAnalysis && batchAnalysis.analyses) {
        Object.entries(batchAnalysis.analyses).forEach(
          ([queryId, analysis]: [string, any]) => {
  ...
  const { data: filtersData, isLoading: filtersLoading } = useQANFilters(
    filtersRequest,
    {
  ...
  const handleRequestSort = (field: string) => {
    ...
    const isCurrentField =
      currentOrderBy === backendField || currentOrderBy === `-${backendField}`;
  ...
  const { totalRow, queryRows } = useMemo(() => {
    ...
    const queries = data.rows.filter(
      (row) =>
        row.fingerprint !== 'TOTAL' && row.dimension !== '' && row.rank > 0
    );
  ...
  const validLoads = queryRows
    .map((row) => getLoadValue(row))
    .filter((load) => !isNaN(load) && load > 0);
  ...
  const handleServiceFilterChange = (
    event: SelectChangeEvent<typeof selectedServicesState>
  ) => {
  ...
  return (
    <Box>
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                {useAIDetection && isAnalyzing ? (
                  ...
                  <>
                    <Typography variant="h5">
                      {anomalyAnalysis.anomalousQueries}/
                      {anomalyAnalysis.totalQueries}
                    </Typography>
                  ...
                  {isAnalyzing ? (
                    ...
                    'overallHealthScore' in anomalyAnalysis && (
                      <Typography
                        variant="h5"
                        color={
                          (anomalyAnalysis as any).overallHealthScore >= 80
                            ? 'success.main'
                            : (anomalyAnalysis as any).overallHealthScore >= 60
                              ? 'warning.main'
                              : 'error.main'
                        }
                      >
      ...
      {anomalyAnalysis.criticalAnomalies > 0 &&
        anomalyAnalysis.topAnomalies.length > 0 && (
          <Paper sx={{ mb: 3 }}>
            <Box sx={{ p: 2 }}>
              <Typography variant="h6" sx={{ mb: 2, color: 'error.main' }}>
                ⚠️ Critical Performance Anomalies Detected
              </Typography>
              {anomalyAnalysis.topAnomalies.filter(
                ({ result }) =>
                  result.overallSeverity === AnomalySeverity.CRITICAL
              ).length > 0 && (
                <Box
                  sx={{
                    p: 2,
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <FormControl sx={{ minWidth: 120 }} size="small">
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        onTimeRangeChange(Number(e.target.value))
                      }
              ...
              {availableServices.length > 1 && (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  {filtersLoading && <CircularProgress size={16} />}
                </Box>
              )}
            </Box>
          </Box>
          ...
          {(selectedServicesState.length > 0 || timeRangeHours) && (
            ...
            >
              ...
              {selectedServicesState.length > 0 && (
                <>
                  <Typography variant="body2" color="textSecondary">
                    Services: {selectedServicesState.length} selected (
                    {selectedServicesState.join(', ')})
                  </Typography>
                  <Button
                    size="small"
                    variant="outlined"
        ...
        <TableContainer>
          {queryRows.length === 0 && selectedServicesState.length > 0 ? (
            <Box sx={{ p: 4, textAlign: 'center' }}>
              ...
              <Typography variant="body2" color="textSecondary">
                No queries match the selected service filter. Try adjusting your
                filter criteria.
              </Typography>
            </Box>
          ) : (
            <>
              <Table>
                ...
                <TableBody>
                  {queryRows.map((row) => {
                    const avgTime =
                      row.metrics?.queryTime?.stats?.avg ||
                      row.metrics?.query_time?.stats?.avg ||
                      0;
                    const maxTime =
                      row.metrics?.queryTime?.stats?.max ||
                      row.metrics?.query_time?.stats?.max ||
                      0;
                    const rowsExamined =
                      row.metrics?.rowsExamined?.stats?.sum ||
                      row.metrics?.rows_examined?.stats?.sum ||
                      row.metrics?.docsExamined?.stats?.sum ||
                      row.metrics?.docs_examined?.stats?.sum ||
                      0;
                    const loadPercentage =
                      maxLoad > 0 ? (getLoadValue(row) / maxLoad) * 100 : 0;
                          ...
                          <Box sx={{ display: 'flex', gap: 0.5 }}>
                            <Tooltip title="Analyze with AI">
                              <IconButton
                                ...
                                color="primary"
                                onClick={() =>
                                  handleAnalyzeInChat(row, row.rank)
                                }
                              >
                            ...
                            <Tooltip title="Get AI Recommendations">
                              <IconButton
                                ...
                                color="secondary"
                                onClick={() =>
                                  handleAnalyzeInPopup(row, row.rank)
                                }
                              >
              ...
              >
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  ...
                  <Typography variant="body2" color="textSecondary">
                    {(page || 0) * (pageSize || 10) + 1}-
                    {Math.min(
                      (page || 0) * (pageSize || 10) + queryRows.length,
                      data.total_rows || 0
                    )}{' '}
                    of {data.total_rows || 0}
                  </Typography>
                </Box>
                <Pagination
