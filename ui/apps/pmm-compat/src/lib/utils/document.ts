export const documentTitleObserver = {
  listen: (callback: (title: string) => void) =>
    new MutationObserver(() => callback(document.title)).observe(
...
export const updateBodyClassByLocation = (location: Location) => {
  const previous = Array.from(document.body.classList).find((className) =>
    className.startsWith('grafana-compat-page')
  );
  if (previous) {
