...
export const DetailsSection: FC = () => {
  ...
  const showTablesTab =
    databaseType !== Databases.mongodb && groupBy === 'queryid' && !totals;
  const showExplainTab =
    databaseType !== Databases.postgresql && groupBy === 'queryid' && !totals;
  ...
  const tabs = [
    {
      ...
      component: (
        <>
          ...
          />
          {metadata ? (
            <Metadata metadata={metadataToShow} loading={metricsLoading} />
          ) : null}
        </>
      ),
    },
