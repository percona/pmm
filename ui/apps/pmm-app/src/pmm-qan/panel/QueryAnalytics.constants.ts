...
export const METRIC_CATALOGUE = {
  ...
  innodb_queue_wait: {
    ...
    tooltipText:
      'Time the query spent either waiting to enter the InnoDB queue, or in it pending execution',
  ...
  wal_bytes: {
    ...
    tooltipText:
      'Total amount of WAL (Write-ahead logging) generated by the statement',
  ...
  locks_global_acquire_count_read_shared: {
    ...
    serviceTypes: [Databases.mongodb],
    metricRelation: (data) => {
      const mainMetric = get(data, [
        'locks_global_acquire_count_read_shared',
        'sum',
      ]);
      const divider = get(data, [
        'locks_global_acquire_count_read_shared',
        'cnt',
      ]);
