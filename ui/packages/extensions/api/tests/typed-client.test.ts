...
import { EXTENSIONS_BASE_PATH } from '../src/base';
import {
  setOnRefreshed,
  setOnUnauthorized,
  setTokenMinter,
...
const API = `http://localhost${EXTENSIONS_BASE_PATH}/api`;

// openapi-fetch builds absolute URLs from a `baseUrl`. The generated paths
// already include PMM Extensions' own `/api/...` prefix, and our typed clients set
// `baseUrl` to an absolute origin plus `EXTENSIONS_BASE_PATH`. Under Node we stub
...
describe('typed-client — PMM Extensions mount point', () => {
  it('resolves a generated path under the prefix nginx exposes the side-car on', async () => {
    const seen = vi.fn();

    server.use(
      http.get('http://localhost/extensions/api/users/me', ({ request }) => {
    ...
    expect(seen).toHaveBeenCalledWith('/extensions/api/users/me');
  });
});
...
describe('typed-client — 401 recovery', () => {
  ...
  it('replays a request body — `fetch` consumed the original stream', async () => {
    ...
    await throwOnApiError(
      extensionsApi.POST('/api/apps/inventory/sync/', {
        body: { syncer: 'mod.Cls' },
      })
    );
