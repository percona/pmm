...
export interface paths {
  '/api/admin/apps/': {
     ...
     *     parent via :attr:`~app.extensions.apps.framework.base.BaseApp.state_key`. The list is
  ...
  '/api/admin/apps/{app_key}/force-disable': {
     ...
     *     (SIGTERM), deletes their :class:`app.extensions.models.AppRunningTask` rows, and
  ...
  '/api/admin/apps/{app_key}/state': {
     ...
     *     re-gated via :func:`app.extensions.periodic_tasks.apply_effective_enabled` so a
     ...
     *     immediately: :func:`app.extensions.app_drain.finalize_drain_if_complete` flips it
  ...
  '/api/apps/': {
     ...
     *     parent via :attr:`~app.extensions.apps.framework.base.BaseApp.state_key`.
     *
     *     :param session: The database session.
  ...
  '/api/apps/alerts/': {
     ...
     *     (:func:`app.extensions.apps.alerts.deps.get_alerts_index_context`) as JSON:
  ...
  '/api/apps/alerts/pagerduty': {
     ...
     *
     *     Mirror :func:`app.extensions.apps.alerts.routes.pagerduty_save` over JSON.
     *
  ...
  '/api/apps/alerts/push': {
     ...
     *
     *     Mirror :func:`app.extensions.apps.alerts.routes.alerts_push` over JSON.
     ...
     *     call :func:`app.extensions.apps.alerts.restore.delete_conflicting_rules` and
     *     retry once.
     *
  ...
  '/api/apps/inventory/sync/': {
     ...
     *     :param internal_token: PMM Extensions internal service token injected by
  ...
  '/api/apps/mysql_backups/services/{service_id}/backups': {
     ...
     *     :data:`~app.extensions.apps.mysql_backups.deps.ResolvedMysqlService`, which lets an
  ...
  '/api/apps/mysql_backups/{task_name}/backups': {
     ...
     *     :data:`~app.extensions.apps.mysql_backups.deps.CataloguedHistoryIds`, so a task that
  ...
  '/api/extensions/admin/settings/export': {
     ...
     *
     *     Aggregates the PMM Extensions wired core settings classes and app-owned settings
     ...
     *     requested classes/keys. Class names and PMM Extensions keys are validated with no
     ...
     *
     *     :param session: The active database session for PMM Extensions override queries.
     *     :type session: AsyncSession
  ...
  '/api/extensions/dashboard/': {
     ...
     *     * ``snippets`` — :meth:`SnippetManager.count` on the PMM Extensions database.
  ...
  '/api/extensions/periodic-tasks/': {
    ...
    /**
     * List Periodic Tasks
     * @description Return the upstream periodic-task page through the PMM Extensions gateway.
     *
  ...
  '/api/extensions/schemas/{schema_id}/tables': {
    ...
    /**
     * List Schema Tables
     * @description Return tables for a schema via the PMM Extensions gateway.
     *
  ...
  '/api/extensions/services/': {
    ...
    /**
     * List Services
     * @description Return a paginated list of inventory services via the PMM Extensions gateway.
     *
     *     :param inventory_api: The Inventory API client used to proxy the request.
     *     :type inventory_api: InventoryAPI
  ...
  '/api/extensions/services/{service_id}/schemas': {
     ...
     * @description Return schemas for a service via the PMM Extensions gateway.
     *
  ...
  '/api/extensions/task-stats/{task_name}': {
     ...
     *     React frontend reaches the data through PMM Extensions rather than calling the Tasks
...
export interface components {
  schemas: {
     ...
     * @description Describe how completely PMM Extensions captured a task's log stream.
     *
    ...
    PaginatedResponse_ExtensionsTaskHistoryResponse_: {
      /** Items */
      items: components['schemas']['ExtensionsTaskHistoryResponse'][];
      /** Limit */
      limit: number;
      /** Offset */
      offset: number;
     ...
     *
     *     Unlike the Form-bound :class:`~app.extensions.snippets.deps.SnippetBatchApproveForm`
     ...
     *
     *     Distinct from :class:`~app.extensions.apps.framework.schema.Capabilities`,
     *     which describes static UI feature flags on
     *     :attr:`~app.extensions.apps.framework.schema.AppSchema.capabilities`
     ...
     *
     *     :param backup_id: Primary key of the :class:`~app.extensions.apps.alerts.models.AlertBackup`
     ...
     *         :func:`~app.extensions.apps.alerts.restore.restore_from_backup`.
     */
     ...
     *     ``Form()`` body, and — via
     *     :func:`~app.extensions.apps.framework.form_dsl.derive_app_schema` — the
     ...
     *     :class:`~app.extensions.apps.framework.schema.AppSchema`: the renderer needs to map
     ...
     *     :class:`~app.extensions.apps.framework.responses.BaseTaskResponse`.
     *
     *     :param hostname: The target hostname for the task execution.
     ...
     *     :func:`~app.extensions.apps.backup_mongo.restore.deps.restore_create_from_write`
  ...
  tasks_list_merged_task_history_api_extensions_task_history__get: {
    ...
    responses: {
      ...
      200: {
        ...
        content: {
          'application/json': components['schemas']['PaginatedResponse_ExtensionsTaskHistoryResponse_'];
        };
      };
      /** @description Validation Error */
      422: {
