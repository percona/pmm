 ...
 *
 * The context lives here, at the root of the PMM Extensions dependency graph, so the
 * framework and every plugin package can read it — they all depend on
 * `@pmm-extensions/api` and none of them may depend on the host application. The provider
 * itself stays in the host (`apps/pmm/src/extensions/ExtensionsAuthProvider.tsx`).
 *
 * Narrower than PMM Extensions' own `AuthSession`, which also carries the user record,
 * the access token and `login`/`logout`. PMM owns the session: the bearer lives
 * in `apps/pmm/src/extensions/extensionsTokenStore.ts` and there is no PMM Extensions login form, so
 ...
 *
 * Deliberately the administrator flag and nothing finer: most unsafe PMM Extensions routes
