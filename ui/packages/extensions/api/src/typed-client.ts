 ...
 *
 * The generated `paths` keys include PMM Extensions' own mount prefix (e.g.
 * `/api/users/me`) but not the `EXTENSIONS_BASE_PATH` the side-car is reached under,
...
import { EXTENSIONS_BASE_PATH } from './base';
import {
  emitUnauthorized,
  getToken,
  isTokenMintRequest,
...
const CLIENT_ORIGIN =
  ...
  typeof (globalThis as { location?: Location }).location?.origin === 'string'
    ...
    : 'http://localhost';
const CLIENT_BASE_URL = `${CLIENT_ORIGIN}${EXTENSIONS_BASE_PATH}`;
