 ...
 * fails this test.
 *
 * Scoped to `ui/packages` — the PMM Extensions derived framework and plugin packages,
...
const REPORTS_ITS_OWN_WAY: Record<string, string> = {
  'extensions/framework/src/components/ScheduledTasksPanel/ScheduledTasksPanel.tsx':
    'panel-level actionError / formError state rendered as an alert above the table',
  'extensions/framework/src/components/TaskHistoryTable/TaskFilesDialog.tsx':
...
describe('mutation failure reporting', () => {
  it('every mutation call site reports its failures in-tree', () => {
    ...
    const sentinels = [
      path.join('extensions', 'framework', 'src', 'index.ts'),
