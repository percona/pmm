...
import { capitalize } from '@pmm-extensions/shared';
...
export function SchemaDrivenPlugin({
  ...
  if (error && !schema) {
    ...
    // ServiceNow credentials are wrong, and PMM Extensions' own wording ("Failed to load
    // plugin schema: Could not validate credentials") named an internal object
    // rather than the thing an operator can fix. Keep on the next PMM Extensions sync.
    return (
      <Box sx={{ py: 4 }}>
