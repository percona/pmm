...
// and, like the real one, ends every append with a newline. The props the
// viewer sets are mirrored onto data attributes so they can be asserted on —
// `follow` in particular decides whether a report opens at its top or at its
// tail, which is not otherwise observable through this stub.
vi.mock('@melloware/react-logviewer', async () => {
  const { forwardRef, useImperativeHandle, useState } = await import('react');
  return {
    LazyLog: forwardRef<
      { appendLines(lines: string[]): void },
      { follow?: boolean; wrapLines?: boolean }
    >(function LazyLog({ follow, wrapLines }, ref) {
...
describe('TaskLogViewer', () => {
  ...
  function streamUrlFor(id: string): string {
    return `/extensions/stream-logs/${id}`;
  }
  ...
  function getEventHandle(id: string): SseStreamHandle {
    const handle = mock.pending
      .filter(
        (h) =>
          h.url.split('?')[0] ===
          `/extensions/stream-logs/${id}/execution-events`
      )
      .at(-1);
    if (!handle) {
  ...
  function logFetchUrls(): string[] {
    return mock.fetchSpy.mock.calls
      ...
      .filter((url) => /^\/extensions\/stream-logs\/[^/?]+(\?|$)/.test(url));
  }
  ...
  it('omits tail param for running tasks', async () => {
    ...
    expect(logFetchUrls()[0]).toBe('/extensions/stream-logs/7');
  ...
  it('keeps a live log that finished cleanly when the run turns terminal', async () => {
    ...
    expect(logFetchUrls()).toEqual(['/extensions/stream-logs/7']);
    expect(screen.getByTestId('log-output').textContent).toBe('line-1\n');
  });
  ...
  it('does not carry a completed live log over to the next history', async () => {
    ...
    expect(logFetchUrls()).toEqual([
      '/extensions/stream-logs/7',
      '/extensions/stream-logs/9',
      '/extensions/stream-logs/9?tail=1000',
    ]);
  });
  ...
  it('forgets a completed live log once it switches to another history', async () => {
    ...
    expect(logFetchUrls()).toEqual([
      '/extensions/stream-logs/7',
      '/extensions/stream-logs/9?tail=1000',
      '/extensions/stream-logs/7?tail=1000',
    ]);
  });
  ...
  it('requests tail=1000 by default for finished tasks', async () => {
    ...
    expect(logFetchUrls()[0]).toBe('/extensions/stream-logs/7?tail=1000');
    expect(getTailSelect()).toHaveTextContent('Last 1000');
  });
  ...
  it('restores the tail choice from localStorage for finished tasks', async () => {
    ...
    expect(logFetchUrls()[0]).toBe('/extensions/stream-logs/3?tail=5000');
  ...
  it('omits tail param when All lines is selected for finished tasks', async () => {
    ...
    await flushPromises();
    expect(logFetchUrls()[0]).toBe('/extensions/stream-logs/5?tail=1000');
    ...
    expect(logFetchUrls().at(-1)).toBe('/extensions/stream-logs/5');
  });
  ...
  it('reconnects with a new tail when the line cap changes for finished tasks', async () => {
    ...
    expect(logFetchUrls().at(-1)).toBe('/extensions/stream-logs/8?tail=100');
    expect(
      globalThis.localStorage.getItem('extensions.taskLogViewer.tail')
    ).toBe('100');
  });
  ...
  it('hides the line cap when a finished log is provably shorter than the smallest option', async () => {
    ...
    expect(logFetchUrls()[0]).toBe('/extensions/stream-logs/20?tail=1000');
  });
  ...
  it('keeps the line cap when a finished pane sits exactly at the requested cap', async () => {
    ...
    await flushPromises();
    expect(logFetchUrls()[0]).toBe('/extensions/stream-logs/22?tail=100');
