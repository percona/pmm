...
describe('useExecutionEvents', () => {
  ...
  describe('running tasks (SSE)', () => {
    it('fetches /extensions/stream-logs/{id}/execution-events with Bearer token', async () => {
      ...
      expect(urlStr).toBe('/extensions/stream-logs/42/execution-events');
  ...
  describe('completed tasks (REST via apiClient)', () => {
    it('calls /extensions/execution-events/{id} via apiClient and surfaces events', async () => {
      ...
      expect(mockApiGet).toHaveBeenCalledWith(
        '/execution-events/7',
        expect.objectContaining({ baseURL: '/extensions' })
      );
