...
describe('useTaskLogs', () => {
  ...
  it('fetches /extensions/stream-logs/{id} with Bearer token', async () => {
    ...
    expect(urlStr).toBe('/extensions/stream-logs/42');
  ...
  it('includes ?tail= when tail is set', async () => {
    ...
    const [url] = mock.fetchSpy.mock.calls[0];
    const urlStr = typeof url === 'string' ? url : (url as URL).href;
    expect(urlStr).toBe('/extensions/stream-logs/42?tail=1000');
  });
  ...
  it('opens a fresh fetch when tail changes', async () => {
    ...
    expect(urlStr).toBe('/extensions/stream-logs/1?tail=1000');
    ...
    expect(urlStr).toBe('/extensions/stream-logs/1?tail=5000');
  });
  ...
  it('opens a fresh fetch when the task id changes', async () => {
    ...
    expect(urlStr).toBe('/extensions/stream-logs/2');
  });
