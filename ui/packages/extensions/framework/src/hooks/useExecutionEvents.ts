...
async function fetchExecutionEvents(
  ...
  // Use apiClient (Bearer interceptor + 401-refresh) with the bare PMM Extensions mount
  // point to reach the legacy /execution-events/{id} route, which unlike the
  // rest of the client's traffic is not under PMM Extensions' own /api prefix.
  const res = await apiClient.get<ExecutionEvent[]>(
    `/execution-events/${taskHistoryId}`,
    {
      baseURL: EXTENSIONS_BASE_PATH,
    }
  );
...
export function useExecutionEvents(
  ...
  useEffect(() => {
    ...
    fetchEventSource(
      `${EXTENSIONS_BASE_PATH}/stream-logs/${idStr}/execution-events`,
      {
        signal: ctrl.signal,
        openWhenHidden: true,
        ...
        onclose: () => {
          ...
          setSseError({
            message: 'Execution events stream connection closed.',
          });
          setSseLoading(false);
        },
      }
