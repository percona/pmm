...
export type TaskExecuteBody =
  ExtensionsComponents['schemas']['framework__TaskExecuteWrite'];
 ...
 *
 * ``GET /api/extensions/task-history/`` returns rows newest-first, so the newest run
 ...
 *
 * The request is routed through the PMM Extensions level plugin gateway
