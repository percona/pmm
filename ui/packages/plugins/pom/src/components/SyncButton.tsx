...
/**
 * Trigger a discovery run and reflect its progress.
 *
 * Owns the whole interaction so both Overview and Discovery get identical
 * behaviour: disable while a run is in flight, poll (via `usePomRuns`) until it
...
export function SyncButton() {
  const { data: runs } = usePomRuns();
  const trigger = useTriggerPomRun();
  const invalidateSnapshot = useInvalidatePomSnapshot();
    ...
    <Stack direction="row" alignItems="center" gap={1}>
      ...
      {conflict && (
        <Typography variant="body2" color="text.secondary">
          A discovery run is already in flight.
        </Typography>
      )}
      {failure && (
