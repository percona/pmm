...
/**
 * Turning POM's estate into rows, as plain functions.
 *
 ...
 */
export function databaseState(host: PomInventoryHost): PomHostDatabaseState {
...
export function repoReachability(
  host: PomInventoryHost
): PomRepoReachability | null {
...
export function toHostRows(
  hosts: PomInventoryHost[] | undefined
): PomHostRow[] {
  if (!hosts) {
    return [];
  }
...
/**
 * Join PMM's snapshot to POM's estate, one row per service.
 *
...
export function joinServiceInventory(
  services: PomServiceRow[],
  inventory: PomInventoryService[] | undefined
): PomServiceInventoryRow[] {
