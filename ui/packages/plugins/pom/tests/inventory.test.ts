...
import type {
  PomInventoryFreshness,
  PomInventoryHost,
  PomInventoryService,
  PomServiceRow,
  PomUnregisteredMongod,
} from '../src/types';

const freshness = (
  overrides: Partial<PomInventoryFreshness> = {}
): PomInventoryFreshness => ({
...
const mongod = (
  overrides: Partial<PomUnregisteredMongod> = {}
): PomUnregisteredMongod => ({
  port: 27018,
...
const service = (
  overrides: Partial<PomInventoryService> = {}
): PomInventoryService => ({
  service_id: 'svc-1',
  node_id: 'node-1',
  name: 'db00',
...
const host = (overrides: Partial<PomInventoryHost> = {}): PomInventoryHost => ({
  node_id: 'node-1',
  name: 'db00',
  address: '10.0.0.1',
  executor_host: 'db00',
...
const snapshotService = (
  overrides: Partial<PomServiceRow> = {}
): PomServiceRow =>
  ({
    service_id: 'svc-1',
    service_name: 'db00',
