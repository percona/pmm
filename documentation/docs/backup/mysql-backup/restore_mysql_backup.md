...
!!! warning "Deprecated"
    This feature is deprecated and will be replaced by [MySQL Backups](../../apps/mysql-backup.md) under **Apps > MySQL Backups** when the [SEP integration](../../apps/index.md) reaches GA. Both options are currently in Tech Preview and run alongside each other. Backups created in one are not visible in the other.
...
2. Click the three dots :material-dots-vertical: in the **Actions** column to check all the information for the backup, then click :material-dots-vertical: **Restore from backup**.
