...
Use this to track snapshot frequency and verify persistence activity. This cumulative counter increments with each successful `BGSAVE` operation, whether triggered automatically by save rules or manually. 

Monitor this to confirm snapshots are occurring as expected and to understand persistence workload over time. Compare the rate of increase with your configured save intervals to ensure automatic snapshots are functioning properly.