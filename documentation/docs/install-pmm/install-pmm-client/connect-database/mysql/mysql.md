...
The MySQL exporter can execute custom queries to collect metrics beyond what PMM provides by default. This lets you monitor any data accessible via SQL — internal statistics, application-level metrics, or business data — and visualize it alongside your standard MySQL metrics in Grafana.

=== "Configuration basics"

    Custom queries must be defined in YAML files placed in this directory on the host where PMM Client is running:

    ```
    /usr/local/percona/pmm/collectors/custom-queries/mysql
    ```

    Three resolution subdirectories control how frequently the queries run:
    ...
    Choose the resolution based on how time-sensitive the data is. All queries in a directory run sequentially within a single connection and must complete within the resolution window — keep individual queries fast.

=== "Query definition format"

    Create a `.yaml` file in the appropriate resolution directory using this structure:
    ...
    The resulting Prometheus metric name is composed of the namespace and column name: `metric_namespace_col2`.
...
=== "Metric types"
    ...
    | `LABEL` | A string dimension, not plotted as a graph | Database name, table name, status |
