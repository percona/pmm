...
1. Note your current PMM Server version. Run `pmm-admin status` on any connected client or check **Configuration > Updates** in the UI.
2. Document your connected databases. Go to **Inventory > Services** and record all monitored services, their connection parameters, and any custom labels.
...
Keep your OVA instance running throughout this process. You will need it as a reference when reconfiguring clients and validating the new setup.

Deploy on a **different host or IP address** from your current OVA, using one of the supported methods:
...
Once your new server is running, reconfigure your clients and restore your settings:
{.power-number}
...
3. Restore any custom dashboards by importing the exported JSON files via **Dashboards > New > Import**.
