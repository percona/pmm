...
OVF virtual appliance support was removed in PMM 3.9.0. If you are still running an older PMM version on a virtual appliance, migrate to a supported deployment method.

The OVF appliance runs PMM Server inside a Docker container, which means you can use the standard PMM backup and restore process to migrate. This preserves dashboards, alert rules, contact points, monitored services, and historical data without manual re-configuration.

## Step 1: Back up PMM Server
Back up the PMM Server data from your OVF instance so you can restore it on the new server:
{.power-number}
...
Deploy PMM Server on a **different host or IP address** from your current OVF instance, using one of the supported methods:
...
Keep your OVF instance stopped while you set up the new server.
...
This restores all monitored services, dashboards, alert rules, and historical data from the OVF instance.
...
2. Decommission the OVF instance once all services are reporting correctly.