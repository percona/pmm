...
| `VMAGENT_loggerLevel` | Logging verbosity level | `WARN`, `ERROR` | `INFO` | Accepts `INFO`, `WARN`, `ERROR`, `FATAL`, `PANIC`; `vmagent` exits on any other value, `DEBUG` included |
| `VMAGENT_promscrape_maxScrapeSize` | Maximum scrape size per target | `128MiB`, `32MiB` | `64MiB` | Controls memory usage |
| `VMAGENT_remoteWrite_basicAuth_username` | Basic auth username for the remote-write endpoint | `vmuser` | Each client's own PMM Server credentials, or the credentials in `PMM_VM_URL` | Overrides the default and replaces it entirely: set both halves, PMM does not complete an injected half with its own credential. Meant for an [external VictoriaMetrics](../../../../reference/third-party/victoria.md#using-victoriametrics-external-database-instance) |
...
Any injected credential, a basic-auth pair, a bearer token, or an OAuth2 client, replaces PMM's default remote-write credential entirely; custom headers and a client TLS certificate are added on top of it. An empty `VMAGENT_*` variable is ignored and logged as a warning at PMM Server startup, because `vmagent` cannot use an empty value.
...
Keep the full `INFO` log (the most detailed level `vmagent` has) with a small disk footprint:

```sh
docker run \
  -e VMAGENT_loggerLevel=INFO \
