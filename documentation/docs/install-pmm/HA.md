...
=== "Manual setup (advanced)"
    ...
    * preserve all data through external clustered storage
    ...
    2.  Create Docker volumes for PMM-Server data:
    ...
    1.  **Access the PMM interface**
        Navigate to `https://<HAProxy_IP>` in your web browser, replacing `<HAProxy_IP>` with the IP address or hostname of the machine running the HAProxy container. HAProxy will automatically route your connection to the current leader PMM instance.

    2.  **Log in to PMM**
        Use the default credentials (`admin`/`admin`) unless changed during setup. PMM will prompt you to set a new password on your first login.

    3.  **Verify HA status**
        Check that your HA setup is functioning correctly:
        ```bash
        # View all running containers
        docker ps

        # Check leader election status
        docker logs pmm-server-active | grep -i leader

        # Verify HAProxy is routing to the leader
        docker logs haproxy | grep -i "selected server"
        ```
        If a container is not running, view its logs using `docker logs <container_name>` to investigate the issue.

    4.  **Register PMM Clients**
        When adding monitored nodes, always use the HAProxy address (or hostname) instead of individual PMM server IPs.
