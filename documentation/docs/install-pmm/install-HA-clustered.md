...
The write endpoint is reachable wherever HAProxy is reachable (see [Configure external access](#configure-external-access)) and is protected by HTTP basic authentication with the shared VictoriaMetrics credentials. Every client presents the same credential, so there is no way to revoke one client on its own: rotating the credential in `pmm-secret` revokes metric writes for all of them at once, and takes effect once the chart has reconfigured `vmauth` and PMM Server with the new value.
