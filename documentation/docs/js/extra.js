document.addEventListener('DOMContentLoaded', function () {
  ...
  for (let i = 0; i < collection.length; i++) {
    ...
    if (commandCodeElementString.startsWith(promptString)) {
      ...
      trueCommand = commandCodeElementString
        .substring(promptString.length, commandCodeElementString.length)
        .trim();
    }
    ...
    if (promptString) {
      if (trueCommand.startsWith(promptString)) {
        trueCommand = trueCommand
          .substring(promptString.length, trueCommand.length)
          .trim();
      }
      trueCommand = trueCommand
        .replaceAll('\n' + promptString, '\n')
    ...
    commandButtonElement
      .item(0)
      .setAttribute('data-clipboard-text', trueCommand);
  }
});
...
window.addEventListener('click', function (e) {
  ...
  const isExternal = /^https?:\/\//i.test(url) && !url.startsWith(location.href);
