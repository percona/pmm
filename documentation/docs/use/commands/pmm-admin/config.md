# pmm-admin configuration commands
...
Stop monitoring a service and remove it from PMM. This removes the service and its agents from PMM Server but does not delete any collected data. Data remains on PMM Server for the configured [retention period](../../reference/faq.md#retention).
...
Annotations appear as vertical lines on Grafana dashboards, helping you correlate performance changes with events. For more details, see [Annotate dashboards](../../use/dashboards-panels/annotate/annotate.md).
...
You can combine `--node`, `--service`, `--node-name`, and `--service-name` to annotate multiple targets at once. If a node or service name is specified, it takes precedence over the auto-detected current node or service.

| Flags | What gets annotated |
|-------|---------------------|
| `--node` | Current node |
