...
Schedule and run MySQL backups for services in your PMM inventory using XtraBackup, Mydumper, or Binlog, restore from backups, and track job status, backup type, and execution history. 

This runs alongside the existing PMM MySQL backup feature and is intended to replace it once the SEP integration reaches GA.   
