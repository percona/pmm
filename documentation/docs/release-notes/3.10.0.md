...
Alongside monitoring, you can now run database operations directly in PMM through the new Management framework. This connects PMM to Percona’s Services Enablement Platform (SEP), which Percona teams use to run database operations without SSH access or extra software. 

In this initial integration, that means running MySQL backups and sending diagnostic results to your Percona Support case: 

#### [**MySQL Backup**](../manage/mysql-backup.md)

Run and schedule MySQL backups using XtraBackup, Mydumper, or Binlog against MySQL services in your PMM inventory. You can also restore backups and track job status, backup type, and execution history. This capability is intended to replace the existing PMM MySQL backup feature when it reaches GA.

![Management MySQL Backup screen](../images/management-mysql-backup.png)

#### [**Support Diagnostics**](../manage/support-diagnostics.md)

Run targeted diagnostic scripts on your monitored hosts and send the results straight to your Percona support case in ServiceNow, without SSH access or manual file uploads.
