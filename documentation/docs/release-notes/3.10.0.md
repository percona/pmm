...
PMM 3.10.0 introduces its first management apps, letting you run database operations directly from PMM without SSH access. These apps connect PMM to Percona's Services Enablement Platform (SEP), the same tool Percona teams use to manage database operations on customer hosts.
