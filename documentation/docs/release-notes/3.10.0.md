...
PMM 3.10.0 introduces its first management apps, letting you run database operations directly from PMM without SSH access. These apps connect PMM to PMM Extensions, the same tool Percona teams use to manage database operations on customer hosts.
...
It runs alongside the existing PMM MySQL backup feature and is intended to replace it once PMM Extensions reaches GA.   
...
To get started, see [Apps](../apps/index.md). For programmatic access, see the [Apps API](https://percona-pmm.readme.io/reference/extensions-tasks-overview).
