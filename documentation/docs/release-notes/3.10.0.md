...
The new Management framework lets you run database operations directly in PMM, without SSH access or extra software. It connects PMM to Percona's Services Enablement Platform (SEP), the same tool Percona teams use to manage database operations on customer hosts. 
...
Schedule and run MySQL backups for services in your PMM inventory using XtraBackup, Mydumper, or Binlog, restore from backups, and track job status, backup type, and execution history. This will replace the existing PMM MySQL backup feature when the SEP integration reaches GA.
