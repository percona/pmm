...
- [PMM-15389](https://perconadev.atlassian.net/browse/PMM-15389): You can now have PMM assume an AWS IAM role to monitor Amazon RDS instead of using static access keys, including a role that lives in a different AWS account. Supply the role ARN in the **AWS IAM role ARN** field on the **Add Instance** page, or with `pmm-admin inventory add agent rds-exporter --aws-role-arn`. See [Assuming an IAM role](../install-pmm/install-pmm-client/connect-database/aws.md#assuming-an-iam-role).
