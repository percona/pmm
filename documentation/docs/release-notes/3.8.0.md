...
**Explore metrics now opt-in**: The **Explore metrics** feature (part of the Grafana Drilldown app) is no longer enabled by default in Grafana 12. If you rely on it to browse and explore metrics visually without writing queries, you need to re-enable it. Go to **Users and Access > Plugins and data > Plugins**, search for **Drilldown**, and enable it. Once enabled, **Explore metrics** reappears under **Explore** in the left sidebar. See [Enable Explore metrics](../reference/ui/ui_components.md#enable-explore-metrics).
