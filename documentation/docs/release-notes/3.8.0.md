...
PMM Dump now supports encrypting dump files with a password, so you can safely share them externally without exposing your monitoring data. Toggle on **Enable encryption** when creating a dataset and enter a password. Encrypted files are saved with an `.enc` suffix. See [Encrypted dumps](../troubleshoot/pmm_dump.md#encrypted-dumps).
...
- [PMM-14441](https://perconadev.atlassian.net/browse/PMM-14441): PMM Dump now supports encrypting dump files with a password. See [Encrypted dumps](../troubleshoot/pmm_dump.md#encrypted-dumps).
