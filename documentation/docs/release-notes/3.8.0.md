...
- [PMM-14934](https://perconadev.atlassian.net/browse/PMM-14934): Fixed an issue where the `GF_SECURITY_ADMIN_USER` and `GF_SECURITY_ADMIN_PASSWORD` environment variables were ignored when starting PMM Server with a named Docker volume (`-v pmm-data:/srv`). You can now set the `admin` username and password at first start using these variables regardless of how the data volume is mounted. As part of this fix, PMM no longer ships with a pre-provisioned `/srv` folder, so the **first start after upgrading takes about 20-30 seconds longer** while stateful components initialize. Subsequent starts are unaffected. This change also reduces the PMM Server image size by about 200 MB. PMM HA deployments are not affected.
