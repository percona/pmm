...
It also delivers enhanced TLS flexibility, improved Nomad GC configuration, and multiple stability and performance fixes.
...
The PostgreSQL exporter has been updated with new collectors for I/O statistics and additional insights into vacuum activity, checkpoint behavior, and parallel query execution. These metrics are automatically gathered when you connect a PostgreSQL 18 instance to PMM.
...
- [PMM-14006](https://perconadev.atlassian.net/browse/PMM-14368): Added full support for PostgreSQL 18, including updated exporters with new collectors for I/O statistics and enhanced metrics for vacuum activity, checkpoint behavior, and parallel query execution. 
...
- [PMM-14498](https://perconadev.atlassian.net/browse/PMM-14498): Fixed an issue where adding external services using `pmm-admin add external` would fail with an internal server error. External services can now be added successfully through both the command line and the user interface.

- [PMM-14444](https://perconadev.atlassian.net/browse/PMM-14444): Fixed duplicate metric collection and type conversion errors in the PostgreSQL exporter that caused excessive logging in PMM agent logs.
...
- [PMM-14243](https://perconadev.atlassian.net/browse/PMM-14260): Fixed an issue preventing PMM Server from starting in OpenShift when using the `namespaceOverride` Helm option.
...
- [PMM-14134](https://perconadev.atlassian.net/browse/PMM-14081): Corrected a typo in the `azure_database_exporter` that prevented monitoring of Azure Database for MySQL – Flexible Server instances. These servers are now properly discovered and monitored.
  
- [PMM-14071](https://perconadev.atlassian.net/browse/PMM-14071): Fixed broken **Service Summary** chart in the **MongoDB Instance Summary** dashboard showed an error instead of data. It now displays metrics correctly.
...
### PMM Server fails to restart on macOS Sequoia with Docker Desktop 4.49.0

[PMM-14438](https://perconadev.atlassian.net/browse/PMM-14438): PMM Server may become inaccessible after Docker container restart on macOS Sequoia 15.7.1 (M2) with Docker Desktop 4.49.0. 

The internal PostgreSQL server fails to shut down gracefully, causing PostgreSQL and Grafana to enter FATAL state and display *"500 Internal Server Error"*.

**Workaround:** Use alternative macOS versions, Linux environments, or cloud deployments instead.