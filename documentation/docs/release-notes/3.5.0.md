...
To get started, see [Connect Valkey databases to PMM](../install-pmm/install-pmm-client/connect-database/valkey-redis.md).
...
PMM 3.5.0 ships with Nomad v1.10.5, which is affected by a vulnerability in Go's crypto library. At the time of this release, Nomad v1.11 which contains the fix was not yet available.
...
Security scanning tools may flag race condition vulnerabilities in the `opencontainers/selinux` dependency but PMM doesn't use untrusted container images or the custom mount configurations required to trigger these vulnerabilities.
