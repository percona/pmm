...
### Changing internal Postgres password may fail due to lock

In rare situations, when you use

```sh
docker exec pmm-server change-admin-password 'admin1'
```
to change the internal Postgres password, you may encounter a database lock error similar to:
...
To resolve this, restart Postgres inside the PMM Server container to release the lock:

```sh
supervisorctl restart postrgesql
```

After restarting, you should be able to change the password again using the command above.
