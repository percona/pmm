...
- [PMM-14440](https://perconadev.atlassian.net/browse/PMM-14440): PMM would show duplicate metric collection errors in `mysqld_exporter` logs due to metrics being registered multiple times. This is now fixed and PMM no longer shows `was collected before with the same name and label values` errors. 
