...
### Some custom alert rules may require datasource update after upgrade

PMM 3.6.0 changes the datasource configuration for alerting. After upgrading, some custom alert rules may fail with `data source not found`. 

If you encounter this error, edit the affected alert rules to update the datasource. For instructions, see [Edit alert rules](../alert/alert_rules.md#edit-alert-rules).
