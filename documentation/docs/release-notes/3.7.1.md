...
- [PMM-14399](https://perconadev.atlassian.net/browse/PMM-14399): Improved documentation for Docker deployments with a new troubleshooting section covering the `FATAL: /srv is not writable for pmm user` error. If your PMM Server container keeps restarting with this error, the [Docker Easy-install guide](../install-pmm/install-pmm-server/deployment-options/docker/easy-install.md) now walks you through fixing it — from correcting volume ownership inside the running container to removing the orphaned data volume if needed.
