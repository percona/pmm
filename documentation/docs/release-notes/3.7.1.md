...
- [PMM-14399](https://perconadev.atlassian.net/browse/PMM-14399): If your PMM Server container keeps restarting with `FATAL: /srv is not writable for pmm user`, the [Docker Easy-install guide](../install-pmm/install-pmm-server/deployment-options/docker/easy-install.md) now has a troubleshooting section that walks you through fixing it — from correcting volume ownership inside the running container to removing the orphaned data volume if needed.
