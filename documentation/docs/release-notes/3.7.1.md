...
It also makes PMM Client setup more flexible for containerized and non-standard environments, and fixes issues in Real-Time Analytics (RTA), upgrade workflows, dashboards, exporters, and documentation.
...
Fixed by upgrading Grafana to 11.6.14. PMM does not enable the `sqlExpressions` feature toggle by default, so PMM was not directly exposed, but the fix reduces any potential risk.

### Go standard library vulnerabilities

We upgraded the Go toolchain to 1.25.8+ across all PMM binaries to fix the following:
...
##### Why this is hard to exploit in PMM
These are Go standard library issues in the ClickHouse Datasource plugin. The plugin only talks to the local ClickHouse instance over localhost — no external or user-controlled URLs, certificates, or TLS connections go through this code.
