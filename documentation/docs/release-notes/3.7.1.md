...
- [PMM-14832](https://perconadev.atlassian.net/browse/PMM-14832): You can now use the `--proc-mounts-path` flag with `pmm-agent setup`, or set `PMM_AGENT_SETUP_PROC_MOUNTS_PATH` as an environment variable, to tell PMM Client where to find `/proc/mounts`. Use this in containerized or non-standard environments where the file is not in its default location and disk metrics appear missing or incorrect.
