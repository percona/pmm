...
With this change, API keys are now deprecated. When you install PMM 3, existing API keys will be automatically converted to service accounts with corresponding service tokens. You can trigger this migration through the UI upon first Admin login or via CLI. For more information see [Migrate to PMM 2 to PMM 3](../pmm-upgrade/migrating_from_pmm_2.md#step-3-migrate-pmm-2-clients-to-pmm-3) and [Service account authentication](../api/authentication.md).
