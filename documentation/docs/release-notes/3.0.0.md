...
| **Upgrade**| [Migrate PMM v2 to PMM v3](../pmm-upgrade/migrating_from_pmm_2.md)
...
## General availability of PMM v3.0.0

We’re excited to announce the General Availability of PMM v3.0.0, a new major version that introduces a complete architectural overhaul of PMM. 
...
When upgrading to PMM v3, any existing unencrypted PMM v2 data will be encrypted automatically.
...
The removal of legacy components like Integrated Alerting and DBaaS, combined with the security enhancements above, eliminates multiple potential vulnerabilities and CVEs. This makes PMM v3 our most secure release yet.
...
PMM v3 introduces a significant upgrade system overhaul, replacing the earlier method based on internal package updates. While the previous approach offered the convenience of upgrading directly via the Update button on the Home dashboard, it was prone to connection issues and package corruption, often resulting in complex recovery challenges.
...
We've optimized the PMM v3 container image from 3.1GB to 2.6GB, reducing its size by 0.5GB. This reduction improves download speeds and enhances deployment reliability, particularly in environments with limited storage capacity.
...
PMM v3 now officially supports ARM architecture, upgrading from its experimental status in PMM v2.43. This means you can reliably monitor databases on ARM platforms, taking advantage of their cost-effective infrastructure and energy efficiency in data centers and cloud environments.
...
PMM now integrates Grafana 11.1.8, which delivers the following important enhancements alongside all the advancements introduced since the previous Grafana 9.2.20 integration in PMM v2.
...
The [MongoDB Router Summary](../reference/dashboards/dashboard-mongodb-router-summary.md), initially introduced as an experimental dashboard in PMM v2, is now generally available in PMM v3. This dashboard provides comprehensive monitoring for MongoS routers in sharded MongoDB clusters, offering insights into MongoS availability, version details and resource utilization. 
...
With Enterprise Linux 7 (EL7) approaching its end-of-life date, we've made sure that PMM v3 exclusively uses Oracle Enterprise Linux 9 (EL9) as the base system for all PMM images.

We began this transition from CentOS 7 to EL9 with the latest PMM v2 releases, and now with PMM v3, we are no longer building Docker containers, AMIs, or OVFs based on EL7.
...
Due to this change, PMM v3 cannot be started on host servers running EL7.
...
While PMM v2.x versions continue to support existing DBaaS functionality, PMM v3 marks the complete deprecation of this feature, removing all references to DBaaS.
...
This release completes the deprecation of Integrated Alerting started in PMM v2.31.0 by removing its remaining components and APIs:
...
- **Grafana 11.1.8**: Includes significant improvements over the previous version 9.2.20 integration in PMM v2.
...
- [PMM-13683](https://perconadev.atlassian.net/browse/PMM-13683) -  Updated `vendor`, `maintainer`, and a few other Docker image labels to meet Red Hat certification requirements, ensuring compatibility with PSMDB Operator 1.19.0 and seamless deployment in OpenShift environments.

## 

!!! seealso alert alert-info "Ready to install or migrate to PMM v3?"
    ...
    - For existing PMM v2 users, we provide a dedicated [Upgrade script]((../pmm-upgrade/migrating_from_pmm_2.md)) that safely migrates your installation to PMM v3 and ensures data is backed up before the upgrade.
