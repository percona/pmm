...
As part of the rootless deployment support, PMM 3 changes the internal ports used by the PMM Server. This port modification is essential for enabling services to run without elevated (root) privileges, further improving security:

- HTTPS: changed from port `443` to `8443`
- HTTP: changed from port `80` to `8080`

This change affects the internal container configuration. If you're using custom scripts or configurations that directly reference the internal ports, you'll need to update them.

For external access, port mappings can still be configured as needed through Docker or Docker Compose. Typical deployments using standard port mappings should be updated to reflect the new internal ports:
