...
When using MySQL Performance Schema as the query source, query examples may show "Sorry, no examples found" depending on your MySQL configuration — including the number of threads, volume of unique queries, and Performance Schema settings — as the history buffer gets overwritten in high-traffic environments.

This affects only the **Examples** tab - all other query metrics (execution time, frequency, resource usage) continue to work normally.

#### Workaround
Configure PMM to use the MySQL slow query log instead of Performance Schema for reliable query examples. For configuration details, see [Limitation: Missing query examples in MySQL Performance Schema](../use/qan/index.md#limitation-missing-query-examples-in-mysql-performance-schema).