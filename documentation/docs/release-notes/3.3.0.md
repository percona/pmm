...
-  [Replication Overview](../reference/dashboards/dashboard-postgresql-replication.md): tracks replication lag and primary-replica relationships
-  [Top Queries](../reference/dashboards/dashboard-postgresql-topqueries.md): detects and optimizes your slowest queries with time-based filtering
-  [Checkpoints, Buffers and WAL Usage](../reference/dashboards/dashboard-postgresql-checkpoints-buffers-wal-usage.md): monitors checkpoint performance with I/O usage correlation
...
-  [Patroni Details](../reference/dashboards/dashboard-postgresql-patroni-details.md): HA cluster monitoring (graduated from experimental)
...
Query Analytics (QAN) now supports label-based access control (LBAC), enabling role-based visibility into query data. Users see only the databases and services permitted by their roles, with filter options automatically restricted based on assigned labels.
...
PMM 3.3.0 introduces `mongolog`, a query source that eliminates connection pool exhaustion by collecting MongoDB metrics directly from log files instead of database queries.
...
To enable mongolog, add `--query-source=mongolog` when registering MongoDB services. For setup instructions, see [Connect MongoDB databases to PMM](../use/connecting-databases/mongodb/mongodb#configure-query-source-for-mongodb.md).
...
- [PMM-13821](https://perconadev.atlassian.net/browse/PMM-13821) - Upgraded Go runtime to version 1.24.x for enhanced security and performance. This update includes the latest CVE fixes and improvements to maintain PMM's security.
...
- [PMM-13881](https://perconadev.atlassian.net/browse/PMM-13881) - Fixed incorrect version reporting by mongodb_exporter. The exporter now displays the correct version number when queried with `--version`.
