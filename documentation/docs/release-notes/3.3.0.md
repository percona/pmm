...
### TLS Skip Verify option for External Services
Added --tls-skip-verify flag support for pmm-admin add external, pmm-admin add external-serverless, and pmm-admin add haproxy commands to enable monitoring of services with self-signed or invalid SSL certificates. This resolves connection issues with IP-based endpoints lacking proper certificate SANs and unblocks PostgreSQL Operator deployments requiring Patroni metrics collection.
