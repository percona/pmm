...
If you're still running PMM 2.x, now is the time to migrate. Starting with PMM 3.8, direct migration to the latest PMM 3.x version will be deprecated and may not work as expected.

If you migrate after PMM 3.8 and run into issues, you can still use PMM 3.7 as a stepping stone since this is the last version where we've fully tested migration from PMM 2.x:

```
PMM 2.x > PMM 3.7 > latest PMM version
```

However, this two-step path will only be available through PMM 3.12. After PMM 3.13 (expected January 2027), you will no longer be able to migrate from PMM 2.x at all.
