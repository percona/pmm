...
PMM includes third-party components, and upstream fixes are not always
available before a release.

For PMM 3.7.0, Percona reviewed the vulnerabilities listed below and assessed
the current risk to most PMM deployments as low.

The following entries explain, for each CVE, which component is affected, why
exploitability is limited in PMM's architecture, and which mitigations apply.

Percona will continue to monitor upstream disclosures, reassess impact on PMM,
and apply dependency updates in future releases.
