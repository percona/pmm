...
#### Deprecated PMM Server from the web interface deprecated

Starting with PMM 3.9, you will no longer be able to upgrade PMM Server from the web interface.

Switch to one of these supported upgrade methods before PMM 3.9  (expected July 2026):
...
#### OVF virtual appliance images deprecated

With this release, PMM Server OVF virtual appliance images are deprecated and will no longer be available after PMM 3.9 (expected July 2026).

Existing OVF-based deployments will continue to work, but new OVA images will no longer be provided after PMM 3.9.
...
Starting with this release, you can no longer upgrade directly from PMM 2.x to the latest PMM 3.x version. Instead, upgrade to PMM 3.7 first, then to the latest version:

```
PMM 2.x > PMM 3.7 > latest PMM version
```
If you are still running PMM 2.x, [Migrate PMM 2 to PMM 3](../pmm-upgrade/migrating_from_pmm_2.html) now.
