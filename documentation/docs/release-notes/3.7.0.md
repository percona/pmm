...
!!! note "MongoDB only"
    Currently supports MongoDB agent types (`mongodb-exporter`, `qan-mongodb-profiler-agent`, `qan-mongodb-mongolog-agent`, `rta-mongodb-agent`). Support for additional agent types is planned for future releases.
...
Starting with PMM 3.9 (expected July 2026), you will no longer be able to upgrade PMM Server from the web interface.

Switch to one of these supported upgrade methods before PMM 3.9:

* [Docker upgrade](../pmm-upgrade/upgrade_docker.html): pull the latest image and restart the container
* [Podman upgrade](../pmm-upgrade/upgrade_podman.html): same container workflow with rootless execution
* [Helm upgrade](../pmm-upgrade/upgrade_helm.html): Kubernetes-native upgrade path

#### Deprecated OVF virtual appliance images 

PMM Server OVF virtual appliance images are deprecated and will no longer be available after PMM 3.9 (expected July 2026).

Existing OVF-based deployments will continue to function, but new OVA images will no longer be provided after PMM 3.9. Migrate to a supported deployment method before then:

* [Docker](../install-pmm/install-pmm-server/deployment-options/docker/index.html) (recommended): simplified deployment and updates
* [Podman](../install-pmm/install-pmm-server/deployment-options/podman/index.html): rootless container execution with enhanced security
* [Kubernetes/Helm](../install-pmm/install-pmm-server/deployment-options/helm/index.html): scalable container orchestration

#### Direct migration from PMM 2.x deprecated

Starting with PMM 3.8, you can no longer upgrade directly from PMM 2.x to the latest PMM 3.x version. Instead, upgrade to PMM 3.7 first, then to the latest version:

```
PMM 2.x → PMM 3.7 → latest PMM version
```

After PMM 3.13 (expected January 2027), migration from PMM 2.x will no longer be supported. If you are still running PMM 2.x, [migrate PMM 2 to PMM 3](../pmm-upgrade/migrating_from_pmm_2.html) now.

## 📦 Components upgrade 

- **VictoriaMetrics**: Upgraded from v1.114.0 to v1.134.0. Adds per-partition index support for improved query performance, along with upstream improvements and fixes. 
- **Grafana**: Upgraded to 11.6.13 to address security vulnerabilities. 
- **Nomad**: Upgraded to 1.11.2 to address security vulnerabilities. 
...
- [PMM-14363](https://perconadev.atlassian.net/browse/PMM-14363): The **Mountpoint usage** graphs on the **Disk Details** dashboard are now shown in gigabytes (GB) instead of bytes, making it easier for you to read and interpret large values.
