...
We deeply appreciate all community security and bug reports that help us identify and fix issues. If you believe you’ve found a security issue, report it via [Percona Security](https://www.percona.com/security).
...
PMM uses third-party components. Sometimes fixes for issues in those components are not ready before a PMM release.

For PMM 3.7.0, Percona reviewed the vulnerabilities listed below and assessed
the risk for most PMM deployments as low.

Each entry shows which component is affected, why the issue is hard to exploit
in PMM, and what mitigations apply.

Percona continues to monitor upstream disclosures, reassess their impact on
PMM, and update dependencies in later releases as fixes become available.

#### gRPC authorization bypass ([CVE-2026-33186](https://nvd.nist.gov/vuln/detail/CVE-2026-33186))

**Affected components**
VictoriaMetrics, HashiCorp Nomad, and Grafana ClickHouse Datasource plugin
(third-party dependencies).

**Risk assessment**
This vulnerability allows authorization bypass when a gRPC server uses
path-based authorization interceptors with deny rules and a fallback allow
rule, and an attacker can send raw HTTP/2 frames with a malformed `:path`
header omitting the leading slash.

PMM does not use gRPC path-based authorization interceptors in any component.
Authentication in PMM is handled through Grafana/nginx HTTP subrequests, not
gRPC-level RBAC. The architectural preconditions required for exploitation do
not exist in PMM deployments.

**Mitigating factors**

- None of the affected components use the `grpc/authz` package or implement
    path-based deny/allow policies in PMM's deployment context.
...
**Risk decision**
Residual risk accepted for PMM 3.7.0. The CVSS base score is Critical, however
the contextual severity for PMM is Low because the attack vector is not
reachable. No upstream tagged releases containing the fix (`grpc >= 1.79.3`)
were available at the time of this release. This issue will be addressed
through upstream dependency updates in a future release.

#### Incorrect IPv6 host literal parsing ([CVE-2026-25679](https://nvd.nist.gov/vuln/detail/CVE-2026-25679))

**Affected component**
Grafana ClickHouse Datasource plugin (third-party dependency, built with Go
1.26.0).

**Risk assessment**
Malformed IPv6 literals in URLs can be parsed incorrectly, potentially
bypassing host validation. The ClickHouse Datasource plugin communicates
exclusively with the local ClickHouse instance over localhost. No external
user-controlled URLs reach this code path.

**Mitigating factors**

- The plugin only connects to ClickHouse on the local network within the PMM
    Server container.
- PMM does not expose the plugin's URL parsing to untrusted input.

**Risk decision**
Residual risk accepted for PMM 3.7.0. This issue will be addressed through
upstream dependency updates in a future release.

#### Incorrect email constraint enforcement ([CVE-2026-27137](https://nvd.nist.gov/vuln/detail/CVE-2026-27137))
...
**Risk assessment**
This issue causes incorrect enforcement of email constraints in x509
certificate validation. The ClickHouse Datasource plugin uses self-signed
certificates for local ClickHouse TLS and does not validate external
certificates with email constraints.

**Mitigating factors**

- The plugin's TLS communication is limited to internal, self-signed
...
#### OpenTelemetry SDK PATH hijacking ([CVE-2026-24051](https://nvd.nist.gov/vuln/detail/CVE-2026-24051))

**Affected component**
Grafana (third-party dependency, `otel/sdk v1.39.0`).

**Risk assessment**
Exploitation requires local filesystem access to the PMM Server container to
manipulate the `PATH` environment variable. PMM Server runs in a locked-down
container with no user shell access. If an attacker already has this level of
access, they have effectively compromised the host.

**Mitigating factors**

- PMM Server is intended to run on trusted infrastructure with restricted
...
#### Docker AuthZ plugin bypass ([CVE-2026-34040](https://nvd.nist.gov/vuln/detail/CVE-2026-34040))

**Affected component**
HashiCorp Nomad (third-party dependency, `docker/moby v28.5.2`).

**Risk assessment**
This vulnerability affects Docker daemon AuthZ plugins when they process
oversized request bodies. Nomad in PMM does not act as a Docker daemon and does
not use Docker AuthZ plugins. Additionally, Nomad is disabled by default in
PMM.

**Mitigating factors**

- Nomad is disabled by default and requires explicit enablement.
- The vulnerable code path (Docker daemon AuthZ) is not exercised by Nomad in
    PMM.
...
- Minimizing the number of PMM administrators and enforcing strong
    authentication.
- Applying resource limits to PMM Server containers where supported.
- Keeping Nomad disabled unless it is explicitly required for your deployment.
