...
The PMM Client configuration file (`pmm-agent.yaml`) now supports optional encryption to protect sensitive credentials stored on disk. 

When you provide an RSA private key, PMM Client automatically encrypts the file using AES-256-GCM and decrypts it at startup.

Encryption is opt-in — existing deployments are not affected.
...
- [PMM-13788](https://perconadev.atlassian.net/browse/PMM-13788) - You can now encrypt the PMM Client configuration file (`pmm-agent.yaml`) to protect credentials at rest. Pass an RSA private key to `pmm-agent`, and it automatically handles encryption and decryption. For details, see [PMM Client configuration file encryption](../admin/security/client_config_encryption.md).
