...
- [PMM-15198](https://perconadev.atlassian.net/browse/PMM-15198): When you monitored MongoDB router (mongos) nodes in a sharded cluster, the exporter kept index and collection statistics from only one shard and dropped the rest. It also flooded the system logs with duplicate `mongodb_indexstats_accesses_ops` metric errors. The exporter now labels these statistics with the shard they come from, so you get complete data from every shard and no duplicate-metric errors. Non-sharded deployments were not affected.
