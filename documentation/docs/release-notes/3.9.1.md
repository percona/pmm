...
- [PMM-15198](https://perconadev.atlassian.net/browse/PMM-15198): If you monitored MongoDB router (mongos) nodes in a sharded cluster, system logs on those nodes were flooded with errors about duplicate `mongodb_indexstats_accesses_ops` metrics, forcing you to unregister the routers from PMM to stop the noise. The MongoDB exporter now handles index stats from mongos nodes correctly.
