...
A high-severity vulnerability has been identified in the Grafana ClickHouse data source that enables arbitrary SQL execution and potential exposure of internal databases and cloud credentials. This vulnerability is tracked as [CVE-YYYY-XXXX](link-to-cve).
...
=== "TEMPORARY: If you cannot upgrade immediately"
    ...
    2. Run the script, replacing `XXXXX` with your PMM admin password. Adjust `CONTAINER`, `PMM_HOST`, and `PMM_PORT` if your setup differs from the defaults:
        ...
        ```sh
        CONTAINER=pmm-server GRAFANA_URL=https://localhost:443 ADMIN_PASS=XXXXX PMM_PORT=443 bash ./pmm-ch-user.sh
        ```
...
- [PMM-15198](https://perconadev.atlassian.net/browse/PMM-15198): Fixed an issue where `mongodb_exporter` flooded system logs with duplicate metric collection errors on MongoDB router (mongos) nodes. The `mongodb_indexstats_accesses_ops` metric was being reported multiple times with identical labels, causing continuous errors in `/var/log/messages` severe enough to require unregistering the router nodes from PMM. The exporter now deduplicates index stats from mongos nodes correctly.
