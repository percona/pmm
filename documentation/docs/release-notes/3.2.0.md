...
- [PMM-13807](https://perconadev.atlassian.net/browse/PMM-13807) - Fixed an issue where `pmm-agent`would crash when processing MySQL slow-log queries containing a column named *Value*. This fix ensures`pmm-agent` properly handles queries with reserved keywords used as column names.
