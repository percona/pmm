...
## Release summary
PMM 3.2.0 comes with better support for Amazon Linux 2023 and MySQL 8.4 replication, an upgrade to Grafana 11.6, secure ClickHouse connections, a new backup dashboard for MongoDB, faster filtering for QAN.  This is in addition to many bug fixes and performance improvements!
...
We've optimized QAN filter loading performance to reduce the number of processed rows by up to 95% in large environments. This means that the filters on the **PMM Query Analytics** page page now load faster, which makes the interface more responsive. 
...
For advanced users, the Nomad API (internal port `4646`) is accessible via the standard PMM HTTPS port using the `/nomad` prefix and requires Administrator privileges.
