...
QAN gets label-based access control and faster filtering. Plus, many bug fixes and performance improvements!
...
QAN now supports label-based access control (LBAC), enabling fine-grained, role-based visibility into query data. This restricts users to viewing only the databases and services assigned to their roles, with filter options automatically adjusted based on their permission labels.
