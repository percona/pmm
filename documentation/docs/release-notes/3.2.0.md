...
### Grafana 11.6 update and upgrade path for external PosgreSQL users
...
!!! caution alert alert-warning "Upgrading to PMM 3.2.0 with an external PostgreSQL database"
    If you're using an external PostgreSQL databas, make sure to update your PMM Server configuration before upgrading to PMM 3.2.0. This is because Grafana 11.6 has a [regression issue](https://github.com/grafana/grafana/issues/102337) where the `GF_DATABASE_URL` environment variable is no longer sufficient for database configuration.  See [Migrate External PostgreSQL Configuration for PMM 3.2.0+](../pmm-upgrade/external_postgres_pmm_upgrade.md) for step-by-step instructions.
