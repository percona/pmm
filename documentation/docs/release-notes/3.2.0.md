...
We've integrated HashiCorp Nomad into PMM, a workload orchestrator that lays the groundwork for future feature development.
...
Nomad is disabled by default, and although the Docker image size has slightly increased, this additional footprint has no impact on performance when Nomad remains disabled. For more information, see [Nomad configuration](../reference/nomad.md). 
...
- [PMM-13958](https://perconadev.atlassian.net/browse/PMM-13958) - Fixed an issue where PMM Server would ignore the `PMM_POSTGRES_*` environment variables and start the internal PostgreSQL instance even when `PMM_DISABLE_BUILTIN_POSTGRES=1` was set. PMM now correctly recognizes the external PostgreSQL configuration and no longer waits for the internal database to start when external settings are provided.
