...
Fixed the built-in **MySQL Down** alert incorrectly resolving to **Normal** when
   both MySQL and the PMM agent go down at the same time. The alert now correctly stays in Firing state even when metrics stop being reported.      
                                          
- [PMM-14661](https://perconadev.atlassian.net/browse/PMM-14661): Fixed an issue where PMM Server would become unresponsive when viewing dashboards with data older than one week.

- [PMM-14193](https://perconadev.atlassian.net/browse/PMM-14193): Fixed the built-in **MySQL Down** alert incorrectly resolving to **Normal** when both MySQL and the PMM agent go down at the same time. The alert now correctly stays in **Firing** state even when metrics stop being reported.      

- [PMM-13910](https://perconadev.atlassian.net/browse/PMM-13910): Fixed the built-in **MySQL Down** alert incorrectly resolving to **Normal** when both MySQL and the PMM agent go down at the same time. The alert now correctly stays in **Firing** state even when metrics stop being reported.                  
