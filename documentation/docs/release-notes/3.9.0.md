...
### One-step PMM Client install ([PMM-14993](https://perconadev.atlassian.net/browse/PMM-14993)) — Dev Preview

Setting up a new database node for monitoring now takes a single command, generated directly from the PMM UI.

The new **Install PMM Client (One-step)** page (under **Inventory** in the sidebar, marked **Preview**) lets you pick a database technology, generate a short-lived service-account token, and copy a ready-to-run command. Run it on your database host and PMM handles the rest:
...
By default, the script prompts for database credentials when run on the terminal — credentials are never embedded in the generated command. For CI or automation, toggle on **Running in CI/automation?** on the page and enter your credentials before copying the command.

For setup instructions, see [Install PMM Client (One-step)](../install-pmm/install-pmm-client/one-click-install.md).
