...
- [PMM-14905](https://perconadev.atlassian.net/browse/PMM-14905): Fixed incorrect `pmm-admin add mongodb` command examples in the documentation. The `--enable-all-collectors` flag was incorrectly included in standard add commands. This flag enables additional collectors that increase overhead and is not recommended for general use. The mongolog command example has also been simplified to include only the required options.
