...
PMM now offers a one-command install for PMM Client. Instead of running through multiple manual steps on every new database host, you can now do it all in one command: install PMM Client, register the node, and add a database service. Combined with the [PMM Server easy-install script](../quickstart/quickstart.md), you can get a full PMM setup with a database under monitoring using just two commands.

The new **Quick-Install PMM Client** page, available under **Inventory** in the sidebar, generates a ready-to-run command. Run it on your database host with `sudo` and PMM Client is installed, the node registered, and your database added for monitoring automatically. You can use it with MySQL, PostgreSQL, MongoDB, and Valkey.

For setup instructions, see [Quick-Install PMM Client](../install-pmm/install-pmm-client/one-click-install.md).
...
- ([PMM-14993](https://perconadev.atlassian.net/browse/PMM-14993)): Added the **Quick-Install PMM Client** page under **Inventory** in the sidebar, providing a one-command install flow for PMM Client.
