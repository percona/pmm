...
- [PMM-14931](https://perconadev.atlassian.net/browse/PMM-14931): PMM Server now includes a built-in ClickHouse configuration optimized for servers with less than 16 GB RAM. 

To activate it, set `PMM_CLICKHOUSE_CONFIG=low-memory` when starting PMM Server. This is useful if you experience Query Analytics (QAN) slowdowns or *"memory limit exceeded"* errors on low-memory hosts. 

The setting persists across container restarts and upgrades without any manual steps. 

If you currently use the `switch-config.sh` script shipped with PMM Server to manage this setting, switch to the environment variable instead. The script is deprecated and will be removed in a future release.
