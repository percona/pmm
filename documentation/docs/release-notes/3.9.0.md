...
### UI upgrade progress bar does not complete when upgrading to PMM 3.9.0

When upgrading to PMM 3.9.0 through the UI, the progress bar may appear stuck. This is because PMM 3.9.0 no longer supports upgrading through the UI, so the page never shows a completed status.

The upgrade runs successfully in the background. Refresh the page to confirm it finished. For future upgrades, see [PMM Server UI upgrade removed](#pmm-server-ui-upgrade-removed).
