...
### UI upgrade progress bar stops when upgrading to PMM 3.9.0

When upgrading to PMM 3.9.0 through the UI, the progress bar stops and displays an `Endpoint not found: /v1/server/updates:getStatus` error. This is because PMM 3.9.0 no longer supports UI-based upgrades.
