...
### One-command PMM Client install ([PMM-14993](https://perconadev.atlassian.net/browse/PMM-14993)) — Dev Preview

Setting up a new database node for monitoring now takes a single command, generated directly from the PMM UI.

The new **Install PMM Client** page (accessible from the sidebar, marked **Preview**) and the **Inventory → Add Node** quick path in Grafana both generate a ready-to-run shell command. Copy it to your database host and run it — PMM handles the rest:

- Installs `pmm-client` using the host's package manager.
- Starts `pmm-agent` (via systemd, or `nohup` as a fallback).
- Registers the node with PMM Server using a short-lived service-account token — no manual Grafana login required on the target host.
- Adds the first monitored service via `pmm-admin add`.

Supported technologies: MySQL, PostgreSQL, MongoDB, and Valkey.

The generated command does not embed database credentials. You can either set them as environment variables before running the command (recommended for automation) or enter them interactively when prompted.

For setup instructions, see [Install PMM Client with one command](../install-pmm/install-pmm-client/one-click-install.md).

> **Dev Preview limitations:**
> Running the script with `--force` on a node that already has services registered removes all existing services on that node. To monitor multiple databases on the same node, run the script separately for each database type.
