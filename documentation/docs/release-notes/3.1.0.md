...
#### New metrics
The **Query Details** section of the QAN dashboard now includes additional metrics:
...
- [PMM-13575](https://perconadev.atlassian.net/browse/PMM-13575) - The `mongodb_pbm_agent_status` metric in MongoDB exporter now includes a `self` label to differentiate between an agent reporting its own status versus reporting status of other agents in the cluster. 

    The `self` label is set to `1` when the agent is reporting about itself, and to `0` when reporting about other agents. This helps identify more precisely which specific agent is experiencing problems without false positives on functioning nodes. 
