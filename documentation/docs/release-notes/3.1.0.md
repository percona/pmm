...
=== "Storage metrics"
    ...
    - **Storage Time Reading Micros**: tracks the time spent on storage read operations, which is useful for detecting potential storage bottlenecks.
...
We have upgraded the [PMM demo environment](https://pmmdemo.percona.com/) from PMM2 to PMM3. The demo provides a fully functional PMM3 interface with pre-configured database monitoring, real-world query examples, and all the latest features. Try it out to explore PMM3 features and interface with sample data.
