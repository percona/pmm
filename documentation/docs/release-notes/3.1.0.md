...
#### Improved MongoDB query fingerprint presentation
We've enhanced how MongoDB query fingerprints are displayed in QAN to improve usability and align with industry standards:

Query fingerprints now show the actual query structure including operators and field names, similar to the format used in MongoDB OpsManager
...
This improvement helps users migrating from other MongoDB monitoring tools who expect to see familiar query representations

This enhancement significantly improves the ability to identify and troubleshoot problematic MongoDB queries in QAN.

#### Enhanced query examples visualization
We've also improved the presentation of query examples in the QAN interface by removing unnecessary enumeration labels ("1 item", "2 item", etc.) and upgrading the JSON viewer component. This creates a cleaner, more focused display that emphasizes the actual query content rather than distracting metadata.
...
- [PMM-12468](https://perconadev.atlassian.net/browse/PMM-12468), [PMM-9288](https://perconadev.atlassian.net/browse/PMM-9288) - Enhanced MongoDB Query Analytics with additional metrics (docs examined, keys examined, locks statistics, storage metrics), new filter categories (plan summary, application name, user, database), and the ability to identify inefficient full collection scans (COLLSCAN) for improved performance troubleshooting.
...
- [PMM-13546](https://perconadev.atlassian.net/browse/PMM-13546) - Alerting - New data source - Incorrect link - [Bug, Pending Release, Matej Kubinec]
- [PMM-13374](https://perconadev.atlassian.net/browse/PMM-13374) - Improved MongoDB query fingerprint presentation in QAN to show detailed query structure similar to OpsManager format, making query patterns more recognizable and enhancing troubleshooting capabilities for users migrating from other MongoDB monitoring tools.
- [PMM-13372](https://perconadev.atlassian.net/browse/PMM-13372) - Improved MongoDB Query Examples presentation in QAN by removing enumeration labels for a cleaner interface.
...
- [PMM-12881](https://perconadev.atlassian.net/browse/PMM-12881) - PMM would trigger both an error notification (explaining the failure) and a misleading success notification when changing PMM settings to an invalid value. This issue has been resolved, and PMM now only shows the appropriate error message when a setting change fails.
- 
