...
We've added new metrics and filters to the [QAN dashboard](../use/qan/index.md) that expose important query execution details, helping highlight inefficient execution paths and providing detailed lock and storage metrics. The new filters make it easy to identify problematic queries based on application, client, user or even plan summary.

#### New MongoDB QAN metrics
The **Query Details** section of the QAN dashboard now additional metrics:

=== "Query execution"
    Metrics that help identify inefficient query patterns:

    - **Docs Examined**: number of documents scanned. High values indicate missing indexes. 
    - **Keys Examined**: number of index keys scanned. Compare this with **Docs Examined** to evaluate index efficiency. 
    - **Docs Returned**: number of documents returned to the client. Examine with high **Docs Examined** for optimization opportunities. 

=== "Response"
    Metrics for evaluating query response efficiency:

    - **Response Length**: size of the response payload in bytes. This is useful for identifying large result sets. 

=== "Lock statistics"
    Metrics for detecting lock contention issues:

    - **Locks Global Acquire Count Read/Write Shared**: frequency of global lock acquisitions. This indicates potential contention points.
    - **Locks Database Acquire Count Read Shared**: database-level read lock acquisitions. 
    - **Locks Database Acquire Wait Count Read Shared**: times a database read lock request had to wait. High values indicate contention. 
    - **Locks Database Time Acquiring Micros Read Shared**: time spent waiting for database read locks. Critical for performance analysis. 
    - **Locks Collection Acquire Count Read Shared**: collection-level read lock acquisitions.

=== "Storage"
    Metrics for analyzing I/O performance:
    
    - **Storage Bytes Read**: volume of data read from storage during operations. This is useful for identifying I/O-intensive queries.
    - **Storage Time Reading Micros**: time spent on storage read operations. This helps identify storage bottlenecks.
...
- **User**: isolate queries by specific database users. 
- **Database**: focus on particular database instances. 

![New Mongo filters](../images/MongoFiltersQAN.png) 

#### Improved query fingerprint visualization
MongoDB query fingerprints in QAN now display the actual query structure, including operators and field names, similar to the format used in MongoDB OpsManager. 

The previous display only showed operation type and collection name, which provided limited context for troubleshooting. The new fingerprint format makes query patterns immediately recognizable while still anonymizing sensitive data. 

![New Mongo query fingerprint](../images/Mongo_fingerprint.png)

#### Enhanced query examples visualization
We've refined the query examples interface in Mongo QAN by removing unnecessary enumeration labels ("1 item", "2 item", etc.) and upgrading the JSON viewer component. This creates a cleaner display that emphasizes the actual query content rather than distracting metadata.

#### Improved MongoDB cluster topology visualization

We have enhanced the [MongoDB Cluster Summary dashboard](../reference/dashboards/dashboard-mongodb-cluster-summary.md) with an redesigned **Current Topology** section. This section now properly categorizes MongoDB cluster components (shards, config servers, and mongos routers) within their appropriate hierarchical sections, replacing the previous implementation in PMM2 that incorrectly grouped all components under **Config Servers**.
...
- [PMM-13545](https://perconadev.atlassian.net/browse/PMM-13545) - Enhanced MongoDB dashboard visualizations with improved node state detection, optimized **Top Hottest Collections** panels, better number formatting, and resolved visualization issues in **Disk I/O** and **Network traffic** panels.
...
- [PMM-13372](https://perconadev.atlassian.net/browse/PMM-13372) - Improved MongoDB **Query Examples** presentation in QAN by removing enumeration labels for a more readable interface.
...
- [PMM-13706](https://perconadev.atlassian.net/browse/PMM-13706) - Fixed an issue where the **Grafana status** panel on the **PMM Health** dashboard incorrectly displayed a **Down** state even when the service was running normally. Indicators now accurately reflect the service state.
