...
=== "Query execution metrics"
    ...
    - **Docs Returned**: The number of documents returned to the client. A high **Docs Examined** to **Docs Returned** ratio may signal optimization opportunities. 
...
=== "Lock statistics metrics"
    ...
    - **Locks Global Acquire Count Read/Write Shared**: Tracks the frequency of global lock acquisitions, helping identify potential contention points.
    - **Locks Database Acquire Count Read Shared**: Measures database-level read lock acquisitions.
    - **Locks Database Acquire Wait Count Read Shared**: Counts instances where a database read lock request had to wait—high values may indicate contention.
    ...
    - **Locks Collection Acquire Count Read Shared**: Tracks collection-level read lock acquisitions. 
...
=== "Storage metrics"
    ...
    - **Storage Time Reading Micros**: Tracks the time spent on storage read operations, useful for detecting potential storage bottlenecks.
...
We have enhanced the [MongoDB Cluster Summary dashboard](../reference/dashboards/dashboard-mongodb-cluster-summary.md) with a redesigned **Current Topology** section. This section now properly categorizes MongoDB cluster components—shards, config servers, and mongos routers—within their appropriate hierarchical sections. This replaces the previous implementation that incorrectly grouped all components under the **Config Servers** section.
