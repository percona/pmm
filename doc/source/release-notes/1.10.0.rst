...
- `New PXC Galera Replication Latency Graphs`_ - Added Galera Replication Latency graphs on PXC
  Overview dashboard with consistent colours

The `issues in the release <https://confluence.percona.com/display/PMM/Release+Notes+1.9.0#ReleaseNotes1.9.0-issuesinrelease>`_ includes 5 new features & improvements, and 6 bugs fixed.
...
(common cases: application deploys, outages, and upgrades) against Database and
...
utilization in the Documentation.

Grafana 5.0
================================================================================

We're extremely pleased to see Grafana ship 5.0 and we were fortunate enough to
be at Grafanacon, including Percona's very own `@Dimitri Vanoverbeke <https://confluence.percona.com/display/~dimitri.vanoverbeke>`_ (Dim0) who presented What we Learned Integrating
...
- :pmmbug:`2295`: Improve colour selection on the *PXC/Galera Cluster Overview*
  dashboard
- :pmmbug:`2371`: Add back host passing when switching between Dashboards
...
* On `Docker Hub <https://hub.docker.com/r/percona/pmm-server/>`_- ``docker pull
  percona/pmm-server`` - `Documentation <https://www.percona.com/doc/percona-monitoring-and-management/deploy/server/docker.html>`_
