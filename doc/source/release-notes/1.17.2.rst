...
One of the improvements in this release is substantial size increase for the AMI images, which allows now  to reserve space for about 10 instances with 30 days retention. Another improvement is the updated Tooltips for the InnoDB Compression Metrics Dashboard by providing a description of what the graphs display, along with links to related documentation resources. An example of Tooltips in action:

.. raw:: html

   <img src="https://www.percona.com/blog/wp-content/uploads/2019/02/GIFMaker.org_tCQcQD.gif" alt="Dashboard Tooltip"> 

PMM 1.17.2 release provides fixes for a number of vulnerabilities: CVE-2019-10747 (set-value package vulnerable to Prototype Pollution), CVE-2019-10746 (mixin-deep vulnerable to Prototype Pollution), CWS-2017-3737 (shelljs vulnerable to Command injection), CVE-2019-15043 (Grafana incorrect access control vulnerability), CVE-2019-8331 (possibility of the cross-site scripting in Bootstrap), CVE-2019-10744 (lodash vulnerable to Prototype Pollution) and WS-2019-0100 (fstream library vulnerable to Arbitrary File Overwrite).

Also PMM 1.17.2 is now based on Grafana 5.4.5.

In release 1.17.2 we have included 2 improvements and fixed 9 bugs.
...
* :pmmbug:`3500`: Update tooltips for the InnoDB Compression Metrics dashboard


Fixed Bugs
================================================================================

* :pmmbug:`511`: Group all metrics on MongoDB Cluster Summary the 'replset' label
...
* :pmmbug:`3256`: Not possible to modify list of collected metrics
* :pmmbug:`3389`: Link of "Advanced Memory Details" was actually leading to the same dashboard
...
* :pmmbug:`4146`: "Error on ingesting out-of-order samples" in Prometheus log when hostname used by PMM dashboards had dash or dot symbol
* :pmmbug:`4596`: Grafana vulnerability allowed partial access to theGrafana HTTP API without being authenticated, making it possbile to run a DoS attack against the server(CVE-2019-15043)
