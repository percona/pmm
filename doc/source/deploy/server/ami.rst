...
2. Login to the |pmm| EC2 instance and verify that the disk capacity has
   increased. From the ``dmesg`` output, you should see something like below::

     [  535.994494] xvdb: detected capacity change from 17179869184 to 34359738368

3. You can check information about logical volumes and volume groups with the
   ``lvs`` and ``vgs`` commands::
...
6. On the previous step, we can see our thin pool is now reflecting the whole
   size of the disk, next is we need to extend the thin volume as well close to
