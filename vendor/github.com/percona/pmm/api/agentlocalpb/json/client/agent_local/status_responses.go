...
type AgentsInfoItems0 struct {
	...
	// Enum: [TYPE_INVALID PMM_AGENT NODE_EXPORTER MYSQLD_EXPORTER MONGODB_EXPORTER POSTGRES_EXPORTER QAN_MYSQL_PERFSCHEMA_AGENT QAN_MYSQL_SLOWLOG_AGENT QAN_MONGODB_PROFILER_AGENT RDS_EXPORTER PROXYSQL_EXPORTER QAN_POSTGRESQL_PGSTATEMENTS_AGENT]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["TYPE_INVALID","PMM_AGENT","NODE_EXPORTER","MYSQLD_EXPORTER","MONGODB_EXPORTER","POSTGRES_EXPORTER","QAN_MYSQL_PERFSCHEMA_AGENT","QAN_MYSQL_SLOWLOG_AGENT","QAN_MONGODB_PROFILER_AGENT","RDS_EXPORTER","PROXYSQL_EXPORTER","QAN_POSTGRESQL_PGSTATEMENTS_AGENT"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
