...
Make sure that the firewall and routing rules of the host do not constrain the Docker container. For more information, see [How do I troubleshoot communication issues between PMM Client and PMM Server?](../faq.md#troubleshoot-connection).

For more information about using Docker, see the [Docker documentation](https://docs.docker.com).

!!! note

    By default, [retention](../faq.md#data-retention) is set to 30 days for Metrics Monitor.  Also consider disabling table statistics, which can greatly decrease Prometheus database size.
...
This step is not required if you are running PMM Server for the first time. However, it ensures that if there is an older version of the image tagged with `2.9.0` available locally, it will be replaced by the actual latest version.
...
!!! note
    ...
    This step creates a new data container based on the latest `percona/pmm-server:2` image. All available versions of `pmm-server` images are listed at [https://hub.docker.com/r/percona/pmm-server/tags/](https://hub.docker.com/r/percona/pmm-server/tags/).
