...
!!! warning "Potential high data volume"
    Enabling all collectors with the `--enable-all-collectors` flag may generate a large volume of data, which could impact disk space usage, especially in high-traffic or large MongoDB environments. Be sure to monitor your disk space after enabling the `--enable-all-collectors` flag.
