...
### New MongoDB collector: Percona Backup for MongoDB (PBM)
The MongoDB exporter now includes a new collector for Percona Backup for MongoDB. This collector gives you visibility to PBM-related metrics from your PBM setups, including `pbm-agent` statuses, PITR configuration, and backup statuses.

To start the MongoDB exporter with the new PBM collector enabled, use the `--enable-all-collectors` flag when adding MongoDB services to your PMM instance:
