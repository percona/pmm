...
- [PMM-12965](https://perconadev.atlassian.net/browse/PMM-12965) - The MongoDB Replication Lag Alert was incorrectly firing for `PRIMARY` nodes and in case `SECONDARY` nodes were down (e.g. for planned maintenance). This has now been fixed by updating the alert expression. The alert will now fire only in case a `SECONDARY` node is up and its lag is above the defined threshold. Users who previously created an alert using this alert rule template should recreate their alert in order to use the updated template.
