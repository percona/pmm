...
To enable the CurrentOp collector when adding MongoDB services to your PMM instance, use the `--enable-all-collectors` flag:

```sh
pmm-admin add mongodb --enable-all-collectors ...
```

There is no need to specify a limit with `--max-collections` for this collector.

Note that the CurrentOp collector may generate a large volume of data, which could impact disk space usage, especially in high-traffic or large MongoDB environments. Be sure to monitor your disk space after enabling this collector.

For more information on configuring and using this new collector and its metrics, see the [pmm-admin commands documentation](../use/commamds/pmm-admin.md).
