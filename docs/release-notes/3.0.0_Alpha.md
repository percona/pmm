...
We've made administration settings easier to manage by introducing a new **Administration** menu. This menu brings together all Grafana-related configurations and account management options that were previously scattered across the general **Configuration** section. This change helps administrators locate and manage Grafana-specific settings more efficiently.

Additionally, we've grouped all PMM settings and inventory options under a new **PMM Configuration** menu. This centralizes access to all PMM-related configurations, making it simpler for users to handle their PMM setup.

These enhancements aim to improve user experience with a more intuitive and organized navigation, allowing you to quickly find and adjust desired settings for both Grafana and PMM.
