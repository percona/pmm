---
name: percona-doc-writer
description: >
  Use this skill for ANY Percona/PMM documentation task. Triggers: "write a new
  ...
  this PR", "add inline comments", "add inline suggestions", "add comments with
  suggestions", "save this to file", "save the draft", "create the file",
  "move this to", "rename this file", "set up release", "prepare release",
  "new release", "scaffold release", "create release notes file",
  "set up PMM X.Y.Z", "prepare docs for release".
---

# Percona Documentation Writer
...
**PR review:**
"review this PR", "review this pull request", "add comments with suggestions",
"give me feedback on this PR", "what do you think of this PR", "add inline
comments", "add inline suggestions", "leave suggestions directly on the files",
"add suggestions directly in the file", "comment on the diff", "add line
comments"

**File operations:**
"save this to file", "save the draft", "create the file", "write this to disk",
"move this to", "rename this file", "update the file", "apply these changes to
the file"
...
```
/Users/catalinaadam/Documents/GitHub/doc-style-guide/docs/
```
...
Use the `view` tool to read each file. Load only what the task requires — do
not load all files for every request.
...
```
/Users/catalinaadam/Documents/GitHub/pmm/documentation/docs/
```

**Do this every time you are writing or editing content, without exception:**

1. Run `view` on the docs root to get the full directory tree
...
read that too and use it as the primary reference. Use `view` with `view_range`
to read specific sections of long files rather than the full file.
...
```
/Users/catalinaadam/Documents/GitHub/pmm/documentation/
```
...
**File operations — saving, moving, renaming:**

Never create, move, or rename a file without explicit user approval of the
content first. The workflow is always: draft in chat → user approves → save to
file.
...
2. Create the file at the approved path using `create_file`
...
1. Read the current file with `view` immediately before making any changes
2. Use `str_replace` for targeted edits; use `create_file` only if rewriting
   the whole file
3. After editing, read the file again with `view` to verify the change is
   correct
...
**Trigger phrases:** "review this PR", "review this pull request", "add comments
with suggestions", "give me feedback on this PR"
...
**Trigger phrases:** "add inline comments", "add inline suggestions", "leave
suggestions directly on the files", "add suggestions directly in the file",
"comment on the diff", "add line comments"
