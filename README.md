...
> The `mkdocs-htmlproofer-plugin` link checking plugin increases build time significantly (~50x). You can temporarily disable it by commenting out the line with `htmlproofer` in `plugins` section of `mkdocs.yml`.
