...
// Package fingerprinter is used to get fingerprint for queries.
...
func (pf *ProfilerFingerprinter) fingerprintFind(fp fingerprinter.Fingerprint, doc proto.SystemProfile) (fingerprinter.Fingerprint, error) {
	...
	if sort, ok := command["sort"]; ok {
		sortJSON, _ := json.Marshal(sort.(bson.D).Map()) //nolint:forcetypeassert,errchkjson // TODO deprecated in GO Driver 2.0
	...
	if _, exists := command["aggregate"]; exists {
		...
		if pipeline, exists := command["pipeline"]; exists {
			...
			for _, stage := range pipelineStages {
				stageMap := stage.(bson.D).Map() //nolint:forcetypeassert,staticcheck // TODO deprecated in GO Driver 2.0
				var stageJSON []byte
				switch {
