...
func TestMySQLShowIndex(t *testing.T) {
	...
	t.Run("Default", func(t *testing.T) {
		...
		case mySQLVendor == version.MariaDBVendor && mySQLVersion.Float() >= 10.5:
			...
			assert.Equal(t, []any{"city", "0", "PRIMARY", "1", "ID", "A", "CARDINALITY", nil, nil, "", "BTREE", "", "", "NO"}, actual[1])
			assert.Equal(t, []any{"city", "1", "CountryCode", "1", "CountryCode", "A", "CARDINALITY", nil, nil, "", "BTREE", "", "", "NO"}, actual[2])
		...
		case mySQLVersion.String() == "5.6" || mySQLVendor == version.MariaDBVendor:
			...
			assert.Equal(t, []any{"city", "0", "PRIMARY", "1", "ID", "A", "CARDINALITY", nil, nil, "", "BTREE", "", ""}, actual[1])
			assert.Equal(t, []any{"city", "1", "CountryCode", "1", "CountryCode", "A", "CARDINALITY", nil, nil, "", "BTREE", "", ""}, actual[2])
		...
		case mySQLVersion.String() == "8.0":
			...
			assert.Equal(t, []any{"city", "0", "PRIMARY", "1", "ID", "A", "CARDINALITY", nil, nil, "", "BTREE", "", "", "YES", nil}, actual[1])
			assert.Equal(t, []any{"city", "1", "CountryCode", "1", "CountryCode", "A", "CARDINALITY", nil, nil, "", "BTREE", "", "", "YES", nil}, actual[2])
