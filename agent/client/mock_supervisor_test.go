...
// RTARequests provides a mock function with no fields
func (_m *mockSupervisor) RTARequests() <-chan *realtimeanalyticsv1.CollectRequest {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for RTARequests")
	}

	var r0 <-chan *realtimeanalyticsv1.CollectRequest
	if rf, ok := ret.Get(0).(func() <-chan *realtimeanalyticsv1.CollectRequest); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
