...
// RealTimeRequests provides a mock function with no fields
func (_m *mockSupervisor) RealTimeRequests() <-chan *realtimev1.RealTimeAnalyticsRequest {
	ret := _m.Called()

	if len(ret) == 0 {
		panic("no return value specified for RealTimeRequests")
	}

	var r0 <-chan *realtimev1.RealTimeAnalyticsRequest
	if rf, ok := ret.Get(0).(func() <-chan *realtimev1.RealTimeAnalyticsRequest); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(<-chan *realtimev1.RealTimeAnalyticsRequest)
		}
	}
